...
struct ComputeVolume
{
  ...
  void operator ()(const blocked_range<vtkIdType> &re)
  {
    ...
    for (vtkIdType cellId = re.begin(); cellId != re.end(); ++cellId) {
      ...
      if (npts == 0) continue;
      irtkAssert(npts == 3, "mesh is triangulated");
...
using namespace irtkPolyhedronUtils;
...
// -----------------------------------------------------------------------------
irtkPolyhedron::irtkPolyhedron(vtkPolyData *polydata)
:
  _DataSet(polydata)
{
}
...
// -----------------------------------------------------------------------------
irtkPolyhedron::irtkPolyhedron(const irtkPolyhedron &other)
:
  irtkObject(other),
...
// -----------------------------------------------------------------------------
irtkPolyhedron &irtkPolyhedron::operator =(const irtkPolyhedron &other)
{
...
// -----------------------------------------------------------------------------
double irtkPolyhedron::Volume(vtkPolyData *polydata)
{
  irtkPolyhedronUtils::ComputeVolume vol;
...
// -----------------------------------------------------------------------------
int irtkPolyhedron::WindingNumber(vtkPolyData *polydata, double x, double y, double z)
{
  irtkPolyhedronUtils::ComputeWindingNumber wn;
