...
#ifndef _IRTKDATASTATISTICS_H
#define _IRTKDATASTATISTICS_H

#include <irtkDataOp.h>


namespace irtk { namespace data { namespace statistic {
...
class Statistic : public Op
{
  ...
  irtkPublicAttributeMacro(string, Description);

  /// Column names for (CSV) output
  irtkPublicAttributeMacro(vector<string>, Names);

  /// Values of data statistic
  irtkReadOnlyAttributeMacro(vector<double>, Values);

protected:

  /// Constructor
  Statistic(const char *desc = NULL, const vector<string> *names = NULL)
  :
...
public:
  ...
  virtual void PrintHeader(ostream &os = std::cout, const char *delimiter = ",") const
  {
  ...
  virtual void PrintValues(ostream &os = std::cout, int digits = 5, const char *delimiter = ",") const
  {
  ...
  virtual void Print(ostream &os = std::cout, int digits = 5, const char *prefix = "") const
  {
    streamsize prev_precision = os.precision(digits);
...
public:
  ...
  Min(const char *desc = "Minimum", const vector<string> *names = NULL)
  :
...
public:
  ...
  Max(const char *desc = "Maximum", const vector<string> *names = NULL)
  :
    Statistic(desc, names)
  {
    if (!names) {
...
public:
  ...
  Extrema(const char *desc = "Extrema", const vector<string> *names = NULL)
  :
    Statistic(desc, names)
  {
    if (!names) {
...
public:
  ...
  Range(const char *desc = "Range", const vector<string> *names = NULL)
  :
    Statistic(desc, names)
  {
    if (!names) {
...
public:
  ...
  Mean(const char *desc = "Mean", const vector<string> *names = NULL)
  :
    Statistic(desc, names)
  {
    if (!names) {
...
class Var : public Statistic
{
  ...
  irtkReadOnlyAttributeMacro(double, Mean);

public:

  Var(const char *desc = "Variance", const vector<string> *names = NULL)
  :
...
public:
  ...
  StDev(const char *desc = "Standard deviation", const vector<string> *names = NULL)
  :
    Var(desc, names)
  {
    if (!names) {
...
public:
  ...
  NormalDistribution(const char *desc = "Normal distribution", const vector<string> *names = NULL)
  :
    StDev(desc, names)
  {
    if (!names) {
...
class Percentile : public Statistic
{
  ...
  irtkReadOnlyAttributeMacro(int, P);

  /// Percentile rank computed as a side product
  irtkReadOnlyAttributeMacro(double, Rank);

public:

  Percentile(int p, const char *desc = NULL, const vector<string> *names = NULL)
  :
    _P(p), _Rank(-1)
  {
    if (!desc) {
...
public:
  ...
  LowerPercentileMean(int p, const char *desc = NULL, const vector<string> *names = NULL)
  :
...
public:
  ...
  UpperPercentileMean(int p, const char *desc = NULL, const vector<string> *names = NULL)
  :
    Percentile(p), _Mean(numeric_limits<double>::quiet_NaN())
  {
    if (!desc) {
...
public:
  ...
  RobustMean(int p, const char *desc = NULL, const vector<string> *names = NULL)
  :
    Percentile(p), _Mean(numeric_limits<double>::quiet_NaN())
  {
    if (!desc) {
