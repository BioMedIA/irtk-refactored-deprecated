...
int main(int argc, char *argv[])
{
  ...
  irtkPolyDataRemeshing::Order melt_order = irtkPolyDataRemeshing::AREA;
  ...
  for (ALL_OPTIONS) {
    if (OPTION("-minedgelength")) {
      ...
      if (arg == "area" || arg == "smallest area" || arg == "smallestarea")
        melt_order = irtkPolyDataRemeshing::AREA;
      else if (arg == "edge" ||
               arg == "shortest" ||
               arg == "shortest edge" ||
               arg == "shortestedge")
        melt_order = irtkPolyDataRemeshing::SHORTEST_EDGE;
      else if (arg == "index" || arg == "none")
        melt_order = irtkPolyDataRemeshing::INDEX;
      else {
  ...
  if (adaptive_name) remesher.AdaptiveEdgeLengthArrayName(adaptive_name);
  ...
  for (size_t i = 0; i < nlevels; ++i) {
    ...
    remesher.Input(mesh);
    remesher.SkipTriangulation(i > 0);
