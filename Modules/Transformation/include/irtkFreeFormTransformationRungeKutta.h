...
#ifndef _IRTKFREEFORMTRANSFORMATIONRUNGEKUTTA_H

#define _IRTKFREEFORMTRANSFORMATIONRUNGEKUTTA_H
...
class irtkFreeFormTransformationRungeKutta
{
protected:

  // -------------------------------------------------------------------------
  /// Helper for computation of Jacobian w.r.t spatial coordinates
  static void dkdx(irtkMatrix &dk, const irtkMatrix &Dv, const irtkMatrix &dx, double h)
  {
  ...
  static void dkdp(irtkMatrix &dk, const irtkMatrix &Dv, const irtkMatrix &dx, const double dv[3], double h)
  {
...
}; // irtkFreeFormTransformationRungeKutta
 ...
 *
 * \sa irtkBSplineFreeFormTransformationTD
 */

template <class TFreeFormTransformation, class TButcherTableau>
class irtkFreeFormTransformationExplicitRungeKutta
:
  public irtkFreeFormTransformationRungeKutta
{
public:
                        ...
                        double t1, double t2, double dt)
  {
    ...
    irtkVector3D<double> k[BT::s];                   // Intermediate evaluations
    const double         d = copysign(1.0, t2 - t1); // Direction of integration
    double               h = d * fabs(dt);           // Initial step size
                       ...
                       double t1, double t2, double dt)
  {
    ...
    irtkVector3D<double> k [BT::s];                  // Intermediate evaluations
    irtkMatrix           dk[BT::s];                  // Derivative of k_i
    irtkMatrix           dx(3, 3);                   // Derivative of intermediate location
    irtkMatrix           Dv(3, 3);                   // Partial derivative of velocity field
    const double         d = copysign(1.0, t2 - t1); // Direction of integration
    double               h = d * fabs(dt);           // Initial step size
    int                  i, j;                       // Butcher tableau indices
    double               l;                          // Temporal lattice coordinate
  ...
  static void JacobianDOFs(const TFreeFormTransformation *v,
                           irtkMatrix &jac, int ci, int cj, int ck, int cl,
                           double &x, double &y, double &z,
                           double t1, double t2, double dt)
  {
    if (t1 == t2) return;
    ...
    irtkVector3D<double> k [BT::s];                  // Intermediate evaluations
    irtkMatrix           dk[BT::s];                  // Derivative of k_i w.r.t control point
    irtkMatrix           dx;                         // Derivative of intermediate location w.r.t control point
    irtkMatrix           Dv;                         // Partial derivative of velocity field w.r.t spatial coordinates
                           ...
                           double t1, double t2, double dt)
  {
    ...
    irtkVector3D<double>       k [BT::s];                  // Intermediate evaluations
    irtkTransformationJacobian dk[BT::s];                  // Derivative of k_i w.r.t transformation parameters
                                                           // and temporarily used to store partial derivative of
                                                           // velocity field w.r.t control points
    irtkTransformationJacobian dx;                         // Current derivative w.r.t transformation parameters
    irtkMatrix                 Dv;                         // Partial derivative of velocity field w.r.t spatial coordinates
    const double               d = copysign(1.0, t2 - t1); // Direction of integration
    double                     h = d * fabs(dt);           // Initial step size
    int                        i, j;                       // Butcher tableau indices
...
}; // irtkFreeFormTransformationExplicitRungeKutta
...
template <class TFreeFormTransformation, class TButcherTableau>
class irtkFreeFormTransformationEmbeddedRungeKutta
:
  public irtkFreeFormTransformationRungeKutta
{
public:
                        ...
                        double t1, double t2, double mindt, double maxdt, double tol)
  {
    ...
    irtkVector3D<double> k[BT::s];                             // k_i = h * v(t + c_i * h, x + sum_{j=0}^{i-1} a_j * k_j)
    irtkVector3D<double> tmp;                                  // Solution of order p-1
    irtkVector3D<double> next;                                 // Solution of order p
    ...
    while (d * t < d * t2) {
      ...
      if (mindt < maxdt) {
        ...
        error = std::max(std::max(fabs(next._x - tmp._x), fabs(next._y - tmp._y)), fabs(next._z - tmp._z));
...
}; // irtkFreeFormTransformationEmbeddedRungeKutta
...
#define IRTK_DECLARE_FFDRK_EXPLICIT(NAME, S) \
  struct irtkFreeFormTransformationButcherTableau##NAME \
  { \
  ...
  class irtkFreeFormTransformationIntegration##NAME \
    : public irtkFreeFormTransformationExplicitRungeKutta \
             <TFreeFormTransformation, irtkFreeFormTransformationButcherTableau##NAME> \
  { }

// ---------------------------------------------------------------------------
/// Declare embedded Runge-Kutta method
#define IRTK_DECLARE_FFDRK_EMBEDDED(NAME, S) \
  struct irtkFreeFormTransformationButcherTableau##NAME \
  { \
    static const int    s = S;   \
    static const int    p;       \
  ...
  template <class TFreeFormTransformation> \
  class irtkFreeFormTransformationIntegration##NAME \
    : public irtkFreeFormTransformationEmbeddedRungeKutta \
             <TFreeFormTransformation, irtkFreeFormTransformationButcherTableau##NAME> \
  { }

// ---------------------------------------------------------------------------
/// Define explicit Runge-Kutta method
#define IRTK_DEFINE_FFDRK_EXPLICIT(NAME, S, P, FSAL, C, A, B) \
  /*const int    irtkFreeFormTransformationButcherTableau##NAME::s       = S;*/    \
  const int    irtkFreeFormTransformationButcherTableau##NAME::p       = P;    \
  const bool   irtkFreeFormTransformationButcherTableau##NAME::fsal    = FSAL; \
  const double irtkFreeFormTransformationButcherTableau##NAME::a[S][S] = A;    \
  const double irtkFreeFormTransformationButcherTableau##NAME::b[S]    = B;    \
  const double irtkFreeFormTransformationButcherTableau##NAME::c[S]    = C

// ---------------------------------------------------------------------------
/// Define embedded Runge-Kutta method
#define IRTK_DEFINE_FFDRK_EMBEDDED(NAME, S, P, FSAL, C, A, BY, BZ) \
  /*const int    irtkFreeFormTransformationButcherTableau##NAME::s       = S;*/        \
  const int    irtkFreeFormTransformationButcherTableau##NAME::p       = P;        \
  const bool   irtkFreeFormTransformationButcherTableau##NAME::fsal    = FSAL;     \
  const double irtkFreeFormTransformationButcherTableau##NAME::a[S][S] = A;        \
  const double irtkFreeFormTransformationButcherTableau##NAME::b[2][S] = {BY, BZ}; \
  const double irtkFreeFormTransformationButcherTableau##NAME::c[S]    = C
...
// ----------------------------------------------------------------------------
IRTK_DECLARE_FFDRK_EXPLICIT(RKE1,   1); ///< Forward Euler method
IRTK_DECLARE_FFDRK_EMBEDDED(RKEH12, 2); ///< Euler-Heun method of order 2(1)
IRTK_DECLARE_FFDRK_EXPLICIT(RKE2,   2); ///< Modified Euler method (Heun's method, explicit midpoint rule)
IRTK_DECLARE_FFDRK_EXPLICIT(RKH2,   2); ///< Improved Euler method (Heun's method, explicit trapezoidal rule)
IRTK_DECLARE_FFDRK_EMBEDDED(RKBS23, 4); ///< Bogacki-Shampine method of order 3(2)
IRTK_DECLARE_FFDRK_EXPLICIT(RK4,    4); ///< Classical Runge-Kutta method
IRTK_DECLARE_FFDRK_EMBEDDED(RKF45,  6); ///< Fehlberg method of order 5(4)
IRTK_DECLARE_FFDRK_EMBEDDED(RKCK45, 6); ///< Cash-Karp method of order 5(4)
IRTK_DECLARE_FFDRK_EMBEDDED(RKDP45, 7); ///< Dormand–Prince method of order 5(4)
...
#ifdef USE_CUDA
#  include <irtkCUFreeFormTransformationRungeKutta.h>
#endif
