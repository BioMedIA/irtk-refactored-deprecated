...
class irtkFluidFreeFormTransformation : public irtkMultiLevelTransformation
{
  irtkTransformationMacro(irtkFluidFreeFormTransformation);

  // ---------------------------------------------------------------------------
  // Attributes

  /// Affine transformation applied after the local free-form deformations.
  irtkPublicAttributeMacro(irtkAffineTransformation, AffineTransformation);
...
public:
  ...
  /// Destructor
  virtual ~irtkFluidFreeFormTransformation();
  ...
  irtkAffineTransformation *GetAffineTransformation();

  /// Get affine transformation applied after the local free-form deformations
  const irtkAffineTransformation *GetAffineTransformation() const;
...
// -----------------------------------------------------------------------------
inline irtkAffineTransformation *irtkFluidFreeFormTransformation::GetAffineTransformation()
{
  return &_AffineTransformation;
}

// -----------------------------------------------------------------------------
inline  const irtkAffineTransformation *irtkFluidFreeFormTransformation::GetAffineTransformation() const
{
  return &_AffineTransformation;
}
...
// -----------------------------------------------------------------------------
inline bool irtkFluidFreeFormTransformation
                                                  ...
                                                  int &i2, int &j2, int &k2, double fraction) const
{
  ...
  //
  // Note: The accuracy of the FFD registration was increased when using
  //       floor and ceil for the rounding instead of fixed up/down casts.
  //       -as12312
  if (version < irtkVersion(3)) {
    i1 = static_cast<int>(x1) + 1;
    i2 = static_cast<int>(x2);
    j1 = static_cast<int>(y1) + 1;
    j2 = static_cast<int>(y2);
    k1 = static_cast<int>(z1) + 1;
    k2 = static_cast<int>(z2);
  } else {
