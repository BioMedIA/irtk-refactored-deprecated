...
#ifndef _IRTKSPARSEMATRIX_H
#define _IRTKSPARSEMATRIX_H
...
class irtkGenericSparseMatrix : public irtkObject
{
  irtkObjectMacro(irtkSparseMatrix);
...
public:
  ...
  /// Storage layout
  irtkReadOnlyAttributeMacro(enum StorageLayout, Layout);
 
  /// Number of rows
  irtkReadOnlyAttributeMacro(int, Rows);

  /// Number of columns
  irtkReadOnlyAttributeMacro(int, Cols);

  /// Number of non-zero entries
  irtkReadOnlyAttributeMacro(int, NNZ);
 
  /// Number of allocated elements
  irtkAttributeMacro(int, Size);

  /// Maximum number of unused entries, i.e., (_Size - _NNZ)
  irtkPublicAttributeMacro(int, MaxNumberOfUnusedEntries);
...
protected:
  ...
  void Copy(const irtkGenericSparseMatrix<TOtherEntry> &);
...
public:
  ...
  /// Default constructor
  irtkGenericSparseMatrix(StorageLayout = CCS);

  /// Construct sparse m x n matrix
  irtkGenericSparseMatrix(int, int = 0, StorageLayout = CCS);

  /// Construct sparse m x n matrix with specified number of non-zero entries
  irtkGenericSparseMatrix(int, int, int, StorageLayout = CCS);

  /// Copy constructor
  template <class TOtherEntry>
  explicit irtkGenericSparseMatrix(const irtkGenericSparseMatrix<TOtherEntry> &);

  /// Assignment operator
  template <class TOtherEntry>
  irtkGenericSparseMatrix &operator =(const irtkGenericSparseMatrix<TOtherEntry> &);

  /// Initialize sparse matrix
  void Initialize(int, int = 0, int = 0);

  /// Initialize sparse matrix given compressed rows (CRS) or columns (CCS)
  void Initialize(int, int, vector<Entries> &, bool = false);
  ...
  /// \note If the input MATLAB array is a sparse matrix, this function is most
  ...
  /// Get diagonal values
  void GetDiag(irtkVector &d) const;

  /// Get diagonal values
  irtkVector Diag() const;

  /// Set diagonal to specified (non-zero) values
  void Diag(const irtkVector &d);

  /// Set diagonal to specified (non-zero) value
  void Diag(TEntry d);

  /// Get non-zero entries of specified submatrix
  irtkGenericSparseMatrix Sub(int, int, int, int) const;

  /// Set non-zero entries of specified submatrix
  void Sub(int, int, const irtkGenericSparseMatrix &);
  ...
  irtkGenericSparseMatrix &operator *=(EntryType);

  /// Divide by a scalar in-place
  irtkGenericSparseMatrix &operator /=(EntryType);

  /// Multiply by a scalar
  irtkGenericSparseMatrix operator *(EntryType) const;
  ...
  /// \note This function is most efficient when the CRS layout is used.
  irtkGenericSparseMatrix &ScaleRow(int, EntryType);

  /// Multiply specified column by a scalar
  /// \note This function is most efficient when the CCS layout is used.
  irtkGenericSparseMatrix &ScaleCol(int, EntryType);

  /// Multiply specified column by a scalar
  /// \note This function is most efficient when the CCS layout is used.
  irtkGenericSparseMatrix &ScaleColumn(int, EntryType);
  ...
  int Eigenvalues(irtkVector &v, int k, const char *sigma = "LM",
                  int p = 0, double tol = .0, int maxit = 0, irtkVector *v0 = NULL) const;
  ...
  ///
  /// \note Only implemented for real double precision sparse matrices.
  int Eigenvectors(irtkMatrix &E, int k, const char *sigma = "LM",
                   int p = 0, double tol = .0, int maxit = 0, irtkVector *v0 = NULL) const;
  ...
  ///
  /// \note Only implemented for real double precision sparse matrices.
  int Eigenvectors(irtkMatrix &E, irtkVector &v, int k, const char *sigma = "LM",
                   int p = 0, double tol = .0, int maxit = 0, irtkVector *v0 = NULL) const;
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::CheckEntries(Entries &entries) const
{
  ...
  for (typename Entries::iterator i = entries.begin(); i != entries.end(); ++i) {
    if (i->second == TEntry(0)) {
      ...
      cerr << "irtkGenericSparseMatrix::CheckEntries: Invalid "
  ...
  for (typename Entries::iterator i = entries.begin(); i != entries.end(); ++i) {
    ...
    entries.erase(i + 1, i + distance(i, j));
  }
}
...
void irtkGenericSparseMatrix<TEntry>::Copy(const irtkGenericSparseMatrix<TOtherEntry> &other)
{
  // Free this matrix
  Deallocate(_Row);
  Deallocate(_Col);
...
// -----------------------------------------------------------------------------
template <class TEntry>
irtkGenericSparseMatrix<TEntry>::irtkGenericSparseMatrix(StorageLayout layout)
:
  _Layout(layout),
  _Rows(0),
  _Cols(0),
...
// -----------------------------------------------------------------------------
template <class TEntry>
irtkGenericSparseMatrix<TEntry>::irtkGenericSparseMatrix(int m, int n, StorageLayout layout)
:
  _Layout(layout),
  _Rows(m),
  _Cols(n ? n : m),
...
// -----------------------------------------------------------------------------
template <class TEntry>
irtkGenericSparseMatrix<TEntry>::irtkGenericSparseMatrix(int m, int n, int nnz, StorageLayout layout)
:
  _Layout(layout),
  _Rows(m),
  _Cols(n ? n : m),
...
// -----------------------------------------------------------------------------
template <class TEntry> template <class TOtherEntry>
irtkGenericSparseMatrix<TEntry>::irtkGenericSparseMatrix(const irtkGenericSparseMatrix<TOtherEntry> &rhs)
:
...
// -----------------------------------------------------------------------------
template <class TEntry> template <class TOtherEntry>
irtkGenericSparseMatrix<TEntry> &irtkGenericSparseMatrix<TEntry>
::operator =(const irtkGenericSparseMatrix<TOtherEntry> &rhs)
{
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Initialize(int m, int n, int sz)
{
  if (n == 0) n = m;
  _Rows = m;
  _Cols = n;
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Initialize(int m, int n, vector<Entries> &entries, bool as_is)
{
  // Free sparse matrix
  Deallocate(_Row);
  Deallocate(_Col);
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Initialize(int m, int n, Entries entries[], bool as_is)
{
  // Free sparse matrix
  Deallocate(_Row);
  Deallocate(_Col);
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Initialize(const mxArray *pm)
{
  // Free sparse matrix
  Deallocate(_Row);
  Deallocate(_Col);
  ...
  if (mxIsSparse(pm)) {
    ...
    cerr << "irtkGenericSparseMatrix::Initialize: Initialization from non-sparse mxArray not implemented" << endl;
    exit(1);
  }
}
#endif // HAVE_MATLAB
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Layout(StorageLayout layout)
{
  if (layout != CRS && layout != CCS) {
    cerr << "irtkGenericSparseMatrix::Layout: Unknown storage layout: " << layout << endl;
    exit(1);
  }
  if (layout != _Layout) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
int irtkGenericSparseMatrix<TEntry>::GetRawData(int *&row, int *&col, TEntry *&data) const
{
  row  = _Row;
  col  = _Col;
  data = _Data;
...
// -----------------------------------------------------------------------------
template <class TEntry>
TEntry *irtkGenericSparseMatrix<TEntry>::RawPointer(int i)
{
  return &_Data[i];
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
const TEntry *irtkGenericSparseMatrix<TEntry>::RawPointer(int i) const
{
  return &_Data[i];
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Reserve(int sz)
{
  if (sz < _NNZ) sz = _NNZ;
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::RemoveZeros()
{
...
// -----------------------------------------------------------------------------
template <class TEntry>
int irtkGenericSparseMatrix<TEntry>::RowNNZ(int r) const
{
  int nnz = 0;
  if (_Layout == CRS) {
    nnz = _Row[r+1] - _Row[r];
...
// -----------------------------------------------------------------------------
template <class TEntry>
int irtkGenericSparseMatrix<TEntry>::ColNNZ(int c) const
{
  int nnz = 0;
  if (_Layout == CCS) {
    nnz = _Col[c+1] - _Col[c];
...
// -----------------------------------------------------------------------------
template <class TEntry>
int irtkGenericSparseMatrix<TEntry>::SubNNZ(int r1, int c1, int r2, int c2) const
{
  int i, j, nnz = 0;
  if (_Layout == CRS) {
    for (int r = r1; r <= r2; ++r) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Index()
{
  if (!_Index) {
    IRTK_START_TIMING();
    int i, j;
    if (_Layout == CRS) {
      _Index = Allocate< vector<int> >(_Cols);
      vector<int> count(_Cols, 0);
      for (i = 0; i < _NNZ;  ++i) {
        ++count[_Col[i]];
      }
      for (j = 0; j < _Cols; ++j) {
      ...
      _Index = Allocate< vector<int> >(_Rows);
      vector<int> count(_Rows, 0);
      for (i = 0; i < _NNZ;  ++i) {
        ++count[_Row[i]];
      }
      for (j = 0; j < _Rows; ++j) {
    ...
    IRTK_DEBUG_TIMING(10, "building sparse matrix index");
  }
}

// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::ClearIndex()
{
  Deallocate(_Index);
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
TEntry &irtkGenericSparseMatrix<TEntry>::operator ()(int r, int c)
{
  if (c < 0) c = r;
  // CCS is equivalent to CRS of transpose matrix
  int rows, *col, *row;
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Put(int r, int c, TEntry v)
{
...
// -----------------------------------------------------------------------------
template <class TEntry>
TEntry irtkGenericSparseMatrix<TEntry>::Get(int r, int c) const
{
  if (c < 0) c = r;
  if (_Layout == CRS) {
    for (int i = _Row[r]; i != _Row[r+1]; ++i) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Row(int r, Entries &row, bool as_is)
{
  if (!as_is) {
    // Remove zero entries
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::GetRow(int r, Entries &row) const
{
  if (_Layout == CRS) {
    row.resize(_Row[r+1] - _Row[r]);
    for (int i = _Row[r], c = 0; i != _Row[r+1]; ++i, ++c) {
      row[c] = make_pair(_Col[i], _Data[i]);
    }
  } else {
    if (_Index) {
      int i, c = 0;
      ...
      for (int c = 0; c < _Cols; ++c) {
        for (int i = _Col[c]; i != _Col[c+1]; ++i) {
          ...
          if (_Row[i] == r) row.push_back(make_pair(c, _Data[i]));
          break;
        }
      }
    }
...
// -----------------------------------------------------------------------------
template <class TEntry>
typename irtkGenericSparseMatrix<TEntry>::Entries
irtkGenericSparseMatrix<TEntry>::Row(int r) const
{
  Entries row;
  GetRow(r, row);
  return row;
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Col(int c, Entries &col, bool as_is)
{
  if (!as_is) {
    // Remove zero entries
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Column(int c, Entries &col, bool as_is)
{
  Col(c, col, as_is);
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::GetCol(int c, Entries &col) const
{
  if (_Layout == CRS) {
    if (_Index) {
      int i, r = 0;
      ...
      for (int r = 0; r < _Rows; ++r) {
        for (int i = _Row[r]; i != _Row[r+1]; ++i) {
          ...
          if (_Col[i] == c) col.push_back(make_pair(r, _Data[i]));
          break;
        }
      }
    }
    ...
    for (int i = _Col[c], r = 0; i != _Col[c+1]; ++i, ++r) {
      col[r] = make_pair(_Row[i], _Data[i]);
    }
  }
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::GetColumn(int c, Entries &col) const
{
  GetCol(c, col);
}
...
template <class TEntry>
typename irtkGenericSparseMatrix<TEntry>::Entries
irtkGenericSparseMatrix<TEntry>::Col(int c) const
{
  Entries col;
  GetCol(c, col);
  return col;
...
template <class TEntry>
typename irtkGenericSparseMatrix<TEntry>::Entries
irtkGenericSparseMatrix<TEntry>::Column(int c) const
{
  return Col(c);
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::GetDiag(irtkVector &d) const
{
  if (_Rows != _Cols) {
    cerr << "irtkGenericSparseMatrix::GetDiag: Matrix must be square" << endl;
    exit(1);
  }
  d.Initialize(_Rows);
...
// -----------------------------------------------------------------------------
template <class TEntry>
irtkVector irtkGenericSparseMatrix<TEntry>::Diag() const
{
  irtkVector d;
  GetDiag(d);
  return d;
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Diag(const irtkVector &d)
{
  if (_Rows != _Cols) {
    cerr << "irtkGenericSparseMatrix::Diag: Matrix must be square" << endl;
    exit(1);
  }
  if (d.Rows() != _Rows) {
    cerr << "irtkGenericSparseMatrix::Diag: Invalid vector size" << endl;
    exit(1);
  }
  // CCS is equivalent to CRS of transpose matrix
  int rows, *col, *row;
  ...
  irtkAssert(i == j, "no more missing diagonal entries");
  while (r >= 0) {
    if (d(r) != .0) {
      // Leave columns above diagonal untouched
      while (i >= row[r] && col[i] > r) --i;
      // Modify diagonal entry of this row
      irtkAssert(i >= row[r] && col[i] == r, "diagonal entry must exist");
      _Data[i] = d(r);
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Diag(TEntry d)
{
  Diag(irtkVector(_Rows, d));
}

// -----------------------------------------------------------------------------
template <class TEntry>
irtkGenericSparseMatrix<TEntry>
irtkGenericSparseMatrix<TEntry>::Sub(int r1, int c1, int r2, int c2) const
{
  irtkGenericSparseMatrix<TEntry> sub(r2 - r1 + 1, c2 - c1 + 1, _Layout);
  cerr << "irtkGenericSparseMatrix::Sub (getter): Not implemented" << endl;
  exit(1);
  return sub;
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>
::Sub(int r1, int c1, const irtkGenericSparseMatrix &sub)
{
  IRTK_START_TIMING();
  int r2 = r1 + sub.Rows() - 1;
  int c2 = c1 + sub.Cols() - 1;
  if (r2 >= _Rows || c2 >= _Cols) {
    cerr << "irtkGenericSparseMatrix::Sub (setter): " << sub.Rows() << " x " << sub.Cols()
  ...
  if (_Layout != sub.Layout()) {
    ...
    cerr << "irtkGenericSparseMatrix::Sub (setter): Submatrix must have same layout as this matrix" << endl;
    exit(1);
  }
  ...
  IRTK_DEBUG_TIMING(7, "irtkGenericSparseMatrix::Sub (setter)");
}

// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Clear()
{
  ClearIndex();
  if (_Layout == CRS) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Zero()
{
  _NNZ = 0;
  if (_Layout == CRS) {
    if (_Row) memset(_Row, 0, (_Rows + 1) * sizeof(int));
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::Transpose(bool keep_layout)
{
  if (keep_layout) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
TEntry irtkGenericSparseMatrix<TEntry>::RowSum(int r) const
{
  TEntry s = TEntry(0);
  if (_Layout == CRS) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
TEntry irtkGenericSparseMatrix<TEntry>::ColSum(int c) const
{
  TEntry s = TEntry(0);
  if (_Layout == CRS) {
    if (_Index) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
TEntry irtkGenericSparseMatrix<TEntry>::ColumnSum(int c) const
{
  return ColSum(c);
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::MultAv(TEntry v[], TEntry w[]) const
{
  const TEntry zero(0);
  if (_Layout == CRS) {
    for (int r = 0; r < _Rows; ++r) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
irtkGenericSparseMatrix<TEntry> &irtkGenericSparseMatrix<TEntry>::operator *=(TEntry s)
{
...
// -----------------------------------------------------------------------------
template <class TEntry>
irtkGenericSparseMatrix<TEntry> irtkGenericSparseMatrix<TEntry>::operator *(TEntry s) const
{
  irtkGenericSparseMatrix<TEntry> m(*this);
  return m *= s;
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
irtkGenericSparseMatrix<TEntry> &irtkGenericSparseMatrix<TEntry>::ScaleRow(int r, TEntry s)
{
  if (_Layout == CRS) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
irtkGenericSparseMatrix<TEntry> &irtkGenericSparseMatrix<TEntry>::ScaleColumn(int c, TEntry s)
{
  if (_Layout == CRS) {
    if (_Index) {
      for (size_t r = 0; r < _Index[c].size(); ++r) {
...
// -----------------------------------------------------------------------------
template <class TEntry>
irtkGenericSparseMatrix<TEntry> &irtkGenericSparseMatrix<TEntry>::ScaleCol(int c, TEntry s)
{
  return ScaleCol(c, s);
}
...
// -----------------------------------------------------------------------------
template <class TEntry>
bool irtkGenericSparseMatrix<TEntry>::IsSymmetric() const
{
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::MakeSymmetric()
{
  if (_Rows != _Cols) {
    cerr << "irtkGenericSparseMatrix::MakeSymmetric: Matrix must be square" << endl;
    exit(1);
  }
  // CCS is equivalent to CRS of transpose matrix
  int rows, *col, *row;
  ...
  for (r = 0; r < rows-1; ++r) {
    ...
    while (i < row[r+1]) {
      ...
      if (j < row[c+1] && col[j] == r) {
        _Data[i] = _Data[j] = .5 * (_Data[i] + _Data[j]);
      } else {
        _Data[i] *= .5;
        for (k = _NNZ; k > j; --k) {
          col  [k] = col  [k-1];
...
// -----------------------------------------------------------------------------
template <class TEntry>
mxArray *irtkGenericSparseMatrix<TEntry>::MxArray() const
{
...
// -----------------------------------------------------------------------------
template <class TEntry>
bool irtkGenericSparseMatrix<TEntry>::ReadMAT(const char *fname, const char *varname)
{
  irtkMatlab::Initialize();
...
// -----------------------------------------------------------------------------
template <class TEntry>
bool irtkGenericSparseMatrix<TEntry>::WriteMAT(const char *fname, const char *varname) const
{
  irtkMatlab::Initialize();
...
// -----------------------------------------------------------------------------
template <class TEntry>
void irtkGenericSparseMatrix<TEntry>::WriteMFile(const char *fname, const char *varname) const
{
  ofstream of(fname);
  // Row indices
  of << varname << "_r = [";
...
typedef irtkGenericSparseMatrix<float>  irtkSparseFloatMatrix;
typedef irtkGenericSparseMatrix<double> irtkSparseDoubleMatrix;

/// Sparse matrix with default entry value type
typedef irtkSparseDoubleMatrix irtkSparseMatrix;
