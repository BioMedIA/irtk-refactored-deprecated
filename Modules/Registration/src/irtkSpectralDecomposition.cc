...
#include <irtkFuzzyCorrespondenceUtils.h>
...
//#ifdef HAVE_MATLAB
//#  define USE_mlxHungarian 1
//#  include <libirtkmwutils.h>
//#endif

using namespace irtk::polydata;


namespace irtkSpectralDecomposition {
...
// -----------------------------------------------------------------------------
inline int NumberOfPoints(vtkPolyData *data, const vector<int> *ids)
{
  return ((ids && !ids->empty()) ? static_cast<int>(ids->size()) : data->GetNumberOfPoints());
}

// -----------------------------------------------------------------------------
inline int PointIndex(vtkPolyData *data, const vector<int> *ids, int i)
{
...
vector<int> RandPerm(int first, int last, int n = -1, int seed = 0)
{
  vector<int> idx(last - first);
...
void GetPoints(irtkPointSet &lp, const irtkPointSet &p, const vector<int> &idx)
{
...
// -----------------------------------------------------------------------------
/// Get points at specified indices
void GetPoints(irtkPointSet &lp, vtkPoints *p, const vector<int> &idx)
{
  const int n = static_cast<int>(idx.size());
  lp.Resize(n);
  double pt[3];
...
void GetEigenmodes(irtkMatrix &lm, const irtkMatrix &m, const vector<int> &idx)
{
  const int n = static_cast<int>(idx.size());
...
void FlipSign(irtkMatrix &m, const vector<int> &idx)
{
  for (vector<int>::const_iterator c = idx.begin(); c != idx.end(); ++c) {
...
class NormalizePoints
{
  ...
  NormalizePoints(irtkPointSet &p) : _Point(p) {}
...
public:
  ...
  static void Run(irtkPointSet &p, irtkPoint *min = NULL, irtkPoint *max = NULL)
  {
    if (p.Size() == 0) return;
...
class RescaleEigenmodes
{
  ...
  RescaleEigenmodes(irtkMatrix &m) : _Matrix(m) {}
...
{
  irtkMatrix       &_Matrix;
  const irtkVector &_Weight;

  ScaleCols(irtkMatrix &m, const irtkVector &w) : _Matrix(m), _Weight(w) {}
...
public:
  ...
  static void Run(irtkMatrix &m, const irtkVector &w)
  {
    ScaleCols scale(m, w);
    blocked_range<int> cols(0, m.Cols());
...
{
  const irtkMatrix           &_E1; /// First eigenmodes
  const irtkMatrix           &_E2; /// Second eigenmodes
  const vector<vector<int> > &_J;  /// Map first row index to second row index
  irtkMatrix                 &_C;  /// Assignment costs
  vector<int>                 _S;  /// Indices of pairs with sign flip

  ComputeAssignmentCosts(const irtkMatrix  &m1, const irtkMatrix &m2,
                         const vector<vector<int> > &r12, irtkMatrix &c12)
  :
...
public:
  ...
  static irtkMatrix Run(const irtkPointSet &p1, const irtkMatrix &m1,
                        const irtkPointSet &p2, const irtkMatrix &m2,
                        const vector<vector<int> > &r12, vector<int> *flip = NULL)
  {
    irtkMatrix w12(m1.Cols(), m2.Cols());
...
// -----------------------------------------------------------------------------
vector<int> WeightedMatching(const irtkMatrix &cost)
{
  vector<int> match(cost.Rows(), -1); // Maps row index to matching column index
  IRTK_START_TIMING();
#if USE_mlxHungarian
  ...
  irtkAssert(mxIsDouble(lhs[0]), "mlxHungarian returns match in matrix of type DOUBLE");
  irtkAssert(static_cast<int>(mxGetM(lhs[0])) == cost.Rows(), "same size as cost matrix");
  irtkAssert(static_cast<int>(mxGetN(lhs[0])) == cost.Cols(), "same size as cost matrix");
  ...
  exit(1);
#endif
  IRTK_DEBUG_TIMING(10, "bipartite matching");
  return match;
}
...
// -----------------------------------------------------------------------------
FindClosestPoints::FindClosestPoints(const irtkPointSet      &p1,
...
// -----------------------------------------------------------------------------
FindClosestNPoints::FindClosestNPoints(const irtkPointSet      &p1,
                                       vtkAbstractPointLocator *p2,
...
// -----------------------------------------------------------------------------
vector<int> FindClosestPoints::Run(const irtkPointSet &p1, const irtkPointSet &p2)
{
  vector<int> idx(p1.Size());
  if (!idx.empty()) {
    vtkSmartPointer<vtkPoints> points;
    points = vtkSmartPointer<vtkPoints>::New();
    points->SetNumberOfPoints(p2.Size());
    for (int i = 0; i < p2.Size(); ++i) {
      const irtkPoint &p = p2(i);
...
// -----------------------------------------------------------------------------
vector<vector<int> > FindClosestNPoints::Run(const irtkPointSet &p1, const irtkPointSet &p2, int num)
{
  vector<vector<int> > idx(p1.Size(), vector<int>(num, -1));
  if (!idx.empty()) {
    vtkSmartPointer<vtkPoints> points;
    points = vtkSmartPointer<vtkPoints>::New();
    points->SetNumberOfPoints(p2.Size());
...
{
  irtkAssert(weights == NULL || weights->size() == 1 || names.size() == weights->size(),
                     ...
                     FeatureWeights               weights)
{
  ...
  // Get edge table
  irtkEdgeTable edgeTable(dataset);
  ...
  for (it.InitTraversal(); it.GetNextEdge(ptId1, ptId2) != -1;) {
    ...
    adjw[r1 + ptId1].push_back(make_pair(c1 + ptId2, w));
    adjw[r1 + ptId2].push_back(make_pair(c1 + ptId1, w));
  }
  ...
  IRTK_DEBUG_TIMING(7, "calculating adjacency weights");
}
...
{
  typedef irtkGenericSparseMatrix<TCorWeight>            WeightMatrix;
      ...
      if (!weight.empty()) {
        ...
        if (wsum > .0) {
          for (WeightIterator w = weight.begin(); w != wend; ++w) {
            ...
            _LnkWeight[r].push_back(make_pair(c, (w->second / wsum) / (d + EPSILON)));
          }
        }
      }
    }
...
void ConnectivityMatrix(SparseMatrix::Entries                      lnkw[],
                        ...
                        int                                        c1,
                        vtkPolyData                               *target,
                        const vector<int>                         *target_sample,
                        vtkPolyData                               *source,
                        const vector<int>                         *source_sample,
                        const irtkGenericSparseMatrix<TCorWeight> *corw,
                        bool                                       transpose)
{
  IRTK_START_TIMING();
  ...
  IRTK_DEBUG_TIMING(7, "calculating connectivity weights");
}

template void ConnectivityMatrix<float>(
  SparseMatrix::Entries [], SparseMatrix::StorageLayout, int, int,
  vtkPolyData *, const vector<int> *, vtkPolyData *, const vector<int> *,
  const irtkGenericSparseMatrix<float> *, bool
);
template void ConnectivityMatrix<double>(
  SparseMatrix::Entries [], SparseMatrix::StorageLayout, int, int,
  vtkPolyData *, const vector<int> *, vtkPolyData *, const vector<int> *,
  const irtkGenericSparseMatrix<double> *, bool
);
...
void ConnectivityMatrix(SparseMatrix                              &lnkw,
                        ...
                        vtkPolyData                               *source,
                        const vector<int>                         *source_sample,
                        const irtkGenericSparseMatrix<TCorWeight> *weight,
                        bool                                       transpose)
{
...
template void ConnectivityMatrix<float >(
  SparseMatrix &, vtkPolyData *, const vector<int> *,
                  vtkPolyData *, const vector<int> *, const irtkGenericSparseMatrix<float> *, bool
);
template void ConnectivityMatrix<double>(
  SparseMatrix &, vtkPolyData *, const vector<int> *,
                  vtkPolyData *, const vector<int> *, const irtkGenericSparseMatrix<double> *, bool
);
...
template SparseMatrix ConnectivityMatrix<float>(
  vtkPolyData *, const vector<int> *, vtkPolyData *, const vector<int> *,
  const irtkGenericSparseMatrix<float> *, bool, SparseMatrix::StorageLayout
);
template SparseMatrix ConnectivityMatrix<double>(
  vtkPolyData *, const vector<int> *, vtkPolyData *, const vector<int> *,
  const irtkGenericSparseMatrix<double> *, bool, SparseMatrix::StorageLayout
);

// -----------------------------------------------------------------------------
void Degree(irtkVector &D, const SparseMatrix &A)
{
  irtkAssert(A.Rows() == A.Cols(), "adjacency matrix must be square");
  const int n = A.Rows();
  D.Initialize(n);
...
// -----------------------------------------------------------------------------
irtkVector Degree(const SparseMatrix &A)
{
  irtkVector D;
  Degree(D, A);
  return D;
...
struct NegateAffinitiesAndDivideByNodeDegree
{
  ...
  NegateAffinitiesAndDivideByNodeDegree(SparseMatrix &A, const irtkVector *D = NULL, const irtkVector *G = NULL)
  :
    _A(&A), _D(D), _G(G && G->Rows() > 0 ? G : NULL), _DOwner(false)
  {
    if (_A->Layout() == SparseMatrix::CCS) _A->Index();
    if (_D == NULL || _D->Rows() == 0) {
      _D      = new irtkVector(_A->Rows());
      _DOwner = true;
      Degree(*const_cast<irtkVector *>(_D), A);
    }
  }
...
void NormalizedLaplacian(SparseMatrix &L, const SparseMatrix &A,
                         const irtkVector *G, const irtkVector *D)
{
  const int n = A.Rows();

  irtkAssert(A.Cols() == A.Rows(), "adjacency matrix must be square");
  irtkAssert(G == NULL || G->Rows() == 0 || G->Rows() == n,
             "weight either given for every node or none at all");
  irtkAssert(D == NULL || D->Rows() == 0 || D->Rows() == n,
  ...
  IRTK_DEBUG_TIMING(7, "calculating the normalized Laplacian");
}
...
{
  irtkAssert(dataset->GetNumberOfPoints() > 1, "dataset must have at least two points");
  ...
  IRTK_DEBUG_TIMING(7, "calculating the graph Laplacian");
}
...
// -----------------------------------------------------------------------------
int ComputeEigenmodes(const SparseMatrix &L, int k, irtkMatrix &m, irtkVector &v)
{
  IRTK_START_TIMING();
  ...
  vector<int>  idx;             // Ascending order of eigenvalues
  irtkMatrix   vec;             // Eigenvectors
  irtkVector   val;             // Eigenvalues
  ...
  IRTK_DEBUG_TIMING(7, "calculating the eigenmodes (#iter=" << niter << ")");
  return k;
}

// -----------------------------------------------------------------------------
int ComputeEigenmodes(vtkPolyData *d, int k, irtkMatrix &m, irtkVector &v,
...
irtkVector MatchEigenmodes(const irtkPointSet &lp1, irtkMatrix       &lm1,
                           const irtkMatrix   &m1,  const irtkVector &v1,
                           const irtkPointSet &lp2, irtkMatrix       &lm2,
                           irtkMatrix         &m2,  irtkVector       &v2,
                           int knn = 5)
{
  irtkAssert(m1.Cols() <= m2.Cols(), "second dataset must have at least as many eigenmodes as first dataset");
  const int k = m1.Cols();
  ...
  // Compute for each pair of eigenmodes and sign flip the assignment cost
  vector<int> flip;
  vector<vector<int> > r12 = FindClosestNPoints::Run(lp1, lp2, 1);
  irtkMatrix w12 = ComputeAssignmentCosts::Run(lp1, lm1, lp2, lm2, r12, &flip);
  // Find optimal assignment
  vector<int> c12 = WeightedMatching(w12);
  // Flip signs
  vector<int> idx(k);
...
// -----------------------------------------------------------------------------
irtkVector MatchEigenmodes(const irtkPointSet &p1, const irtkMatrix &m1, const irtkVector &v1,
                           const irtkPointSet &p2,       irtkMatrix &m2,       irtkVector &v2,
                           int ratio, int knn)
{
  // Downsample points
  //
...
#else
  irtkPointSet lp1(p1), lp2(p2);
  irtkMatrix   lm1(m1), lm2(m2);
#endif
  // Match eigenmodes
...
// -----------------------------------------------------------------------------
irtkVector MatchEigenmodes(vtkPoints *p1, const irtkMatrix &m1, const irtkVector &v1,
                           vtkPoints *p2,       irtkMatrix &m2,       irtkVector &v2,
                           int ratio, int knn)
{
  // Downsample points
  //
...
#else
  irtkMatrix lm1(m1), lm2(m2);
  irtkPointSet lp1(static_cast<int>(p1->GetNumberOfPoints()));
  irtkPointSet lp2(static_cast<int>(p2->GetNumberOfPoints()));
  double p[3];
...
// -----------------------------------------------------------------------------
void WeightEigenmodes(const irtkMatrix &m1, const irtkVector &v1,
                      const irtkMatrix &m2, const irtkVector &v2, irtkVector &w)
{
  irtkAssert(m1.Cols() <= m2.Cols(), "second dataset must have at least as many eigenmodes as first dataset");
  const int k = w.Rows();
  double wsum = .0;
...
// -----------------------------------------------------------------------------
void NormalizeEigenmodes(irtkMatrix &m)
{
  RescaleEigenmodes::Run(m);
}

// -----------------------------------------------------------------------------
void ScaleEigenmodes(irtkMatrix &m, const irtkVector &w)
{
  ScaleCols::Run(m, w);
}

// -----------------------------------------------------------------------------
void ScaleEigenmodes(irtkMatrix &m1, irtkMatrix &m2, const irtkVector &w)
{
  ScaleCols::Run(m1, w);
  ScaleCols::Run(m2, w);
}
...
// -----------------------------------------------------------------------------
vtkSmartPointer<vtkDataArray> ToPointDataArray(const irtkMatrix &m, int r1, int n, int k, const char *name)
{
  if (n <  0) n = m.Rows();
  if (k <= 0 || k > m.Cols()) k = m.Cols();
  irtkAssert(r1 + n <= m.Rows(), "spectral components given for each point");
...
// -----------------------------------------------------------------------------
int SetEigenmodes(vtkPolyData *d, const irtkMatrix &m, const char *name)
{
  return SetEigenmodes(d, m, 0, -1, name);
}

// -----------------------------------------------------------------------------
int SetEigenmodes(vtkPolyData *d, const irtkMatrix &m, int r1, int k, const char *name)
{
...
// -----------------------------------------------------------------------------
irtkMatrix GetEigenmodes(vtkPolyData *d, const char *name)
{
  irtkAssert(name != NULL, "array name must be specified");
  vtkDataArray *s = d->GetPointData()->GetArray(name);
  if (s == NULL) return irtkMatrix();
