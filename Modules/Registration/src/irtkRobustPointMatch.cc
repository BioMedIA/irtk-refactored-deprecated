...
namespace irtkRobustPointMatchUtils {
...
public:
  ...
  void operator()(const blocked_range<int> &re)
  {
    ...
    for (int r = re.begin(); r != re.end(); ++r) {
      _Target->GetPoint(irtkPointCorrespondence::GetPointIndex(_Target, _Sample, r), p1);
  ...
  void Add(vtkPointSet       *target,
           ...
           int                num = 0)
  {
    const int m = irtkPointLocator::GetNumberOfPoints(target, sample);
  ...
  void Add(vtkPointSet *target, const vector<int> *sample, vtkPointSet *source, double source_frac, int min_num  = 1)
  {
    int num = max(min_num, round(source_frac * source->GetNumberOfPoints()));
...
{
  typedef irtkRobustPointMatch::WeightMatrix WeightMatrix;
  typedef irtkRobustPointMatch::FeatureList  FeatureList;

  const irtkRegisteredPointSet *_Target;
  const vector<int>            *_TargetSample;
  const FeatureList            *_TargetFeatures;
  const irtkRegisteredPointSet *_Source;
...
public:
  ...
  void operator()(const blocked_range<int> &re) const
  {
    ...
    for (int i = re.begin(); i != re.end(); ++i) {
      irtkPointCorrespondence::GetPoint(p1, _Target, _TargetSample, i, _TargetFeatures);
      ...
      if (ids->GetNumberOfIds() > 0) {
        ...
        for (vtkIdType j = 0; j < ids->GetNumberOfIds(); ++j, ++weight) {
          irtkPointCorrespondence::GetPoint(p2, _Source, _SourceSample, ids->GetId(j), _SourceFeatures);
          weight->first  = irtkPointCorrespondence::GetPointIndex(_Source, _SourceSample, ids->GetId(j));
          weight->second = exp(- vtkMath::Distance2BetweenPoints(p1, p2) / _Temperature);
          if (_NumberOfFeatures > 3) {
            weight->second *= exp(- irtkPointCorrespondence::Distance2BetweenPoints(p1+3, p2+3, _NumberOfFeatures-3) / _VarianceOfFeatures);
          }
        }
      }
    }
  ...
  {
    if (min_weight >= 1.0) {
      cerr << "irtkRobustPointMatch::CalculateWeights: Minimum weight must be less than 1" << endl;
      exit(1);
    }
    if (min_weight <= .0) {
      cerr << "irtkRobustPointMatch::CalculateWeights: Minimum weight must be positive" << endl;
      exit(1);
    }
    ...
    const int m = irtkPointCorrespondence::GetNumberOfPoints(target, target_sample);
    const int n = irtkPointCorrespondence::GetNumberOfPoints(source, source_sample);
    if (m == 0 || n == 0) return;
    IRTK_START_TIMING();
    ...
    body._NumberOfFeatures   = irtkPointCorrespondence::GetPointDimension(target, target_features);
    ...
    dataset = irtkPointCorrespondence::GetPointSet(source, source_sample);
    ...
    IRTK_DEBUG_TIMING(7, "calculating weight for each pair of points");
  }
};
...
public:
  ...
  void operator ()(const blocked_range<int> &re)
  {
    double p[3];
    for (int i = re.begin(); i != re.end(); ++i) {
      _DataSet->GetPoint(irtkPointCorrespondence::GetPointIndex(_DataSet, _Sample, i), p);
      _Sum += p;
    }
  }
  ...
  static void Run(vtkPointSet       *dataset,
                  ...
                  irtkPoint         &centroid)
  {
    const int n = irtkPointCorrespondence::GetNumberOfPoints(dataset, sample);
    if (n == 0) return;
    IRTK_START_TIMING();
    ...
    IRTK_DEBUG_TIMING(7, "getting centroid of points");
  }
};
...
public:
  ...
  void operator()(const blocked_range<int> &re) const
  {
    ...
    for (int i = re.begin(); i != re.end(); ++i) {
      ...
      _CorrWeights[i].push_back(make_pair(_N, weight));
    }
  }

  static void Run(const irtkRegisteredPointSet *dataset,
                  const vector<int>            *sample,
                  const irtkPoint             &cluster,
                  WeightMatrix::Entries        *corrw,
                  int                           n,
                  double                        temperature)
  {
    const int m = irtkPointCorrespondence::GetNumberOfPoints(dataset, sample);
    if (m == 0) return;
    IRTK_START_TIMING();
    ...
    parallel_for(i, body);
    IRTK_DEBUG_TIMING(7, "calculating outlier weights 1");
  }
};
...
public:
  ...
  void operator()(const blocked_range<int> &re) const
  {
    ...
    for (int i = re.begin(); i != re.end(); ++i) {
      _DataSet->GetPoint(irtkPointCorrespondence::GetPointIndex(_DataSet, _Sample, i), p);
      weight = exp(- p.SquaredDistance(_Cluster) / _Temperature);
      (*_CorrWeights)[i] = make_pair(i, weight);
    }
  }
                  ...
                  double                        temperature)
  {
    ...
    blocked_range<int> i(0, n);
    parallel_for(i, body);
    IRTK_DEBUG_TIMING(7, "calculating outlier weights 2");
  }
};


} // namespace irtkRobustPointMatchUtils
using namespace irtkRobustPointMatchUtils;
...
// -----------------------------------------------------------------------------
irtkRobustPointMatch::irtkRobustPointMatch()
:
...
// -----------------------------------------------------------------------------
irtkRobustPointMatch::irtkRobustPointMatch(const irtkRegisteredPointSet *target,
                                           const irtkRegisteredPointSet *source)
:
  _InitialTemperature(numeric_limits<double>::quiet_NaN()),
  _AnnealingRate     (numeric_limits<double>::quiet_NaN()),
...
// -----------------------------------------------------------------------------
irtkRobustPointMatch::irtkRobustPointMatch(const irtkRobustPointMatch &other)
:
...
// -----------------------------------------------------------------------------
irtkPointCorrespondence *irtkRobustPointMatch::NewInstance() const
{
  return new irtkRobustPointMatch(*this);
}
...
// -----------------------------------------------------------------------------
irtkRobustPointMatch::TypeId irtkRobustPointMatch::Type() const
{
  return RobustPointMatch;
}
...
// -----------------------------------------------------------------------------
bool irtkRobustPointMatch::Set(const char *name, const char *value)
{
  // Initial temperature
...
// -----------------------------------------------------------------------------
irtkParameterList irtkRobustPointMatch::Parameter() const
{
...
// -----------------------------------------------------------------------------
void irtkRobustPointMatch::Initialize()
{
  // Initialize base class
  irtkFuzzyCorrespondence::Initialize();
...
// -----------------------------------------------------------------------------
void irtkRobustPointMatch::InitializeAnnealing()
{
  IRTK_START_TIMING();
  ...
  if (IsNaN(_InitialTemperature) || _InitialTemperature <= .0) {
    IRTK_LOG_EVENT("Initializing annealing process...\n");
    ...
    IRTK_LOG_EVENT("  Considering " << k << " nearest neighors\n");
    ...
    IRTK_LOG_EVENT("  Mean squared distance = " << dist2.Mean() << " (sigma = " << dist2.Sigma() << ")\n");
    ...
    IRTK_LOG_EVENT("Initializing annealing proces... done\n");
  }
  ...
  IRTK_LOG_EVENT("Initial temperature = " << _InitialTemperature << "\n");
  IRTK_LOG_EVENT("Final   temperature = " << _FinalTemperature   << "\n");

  // Set initial temperature
  _Temperature = _InitialTemperature;
  IRTK_LOG_EVENT("Temperature = " << _InitialTemperature << "\n");

  IRTK_DEBUG_TIMING(6, "initialization of annealing process");
}

// -----------------------------------------------------------------------------
bool irtkRobustPointMatch::Upgrade()
{
  // Have base class check ratio of outliers
  if (!irtkFuzzyCorrespondence::Upgrade()) return false;
  ...
  if (_Temperature < _FinalTemperature) {
    IRTK_LOG_EVENT("Final temperature reached\n");
    return false;
  }

  // Log current temperature
  IRTK_LOG_EVENT("Temperature = " << _Temperature << "\n");
  return true;
}

// -----------------------------------------------------------------------------
void irtkRobustPointMatch::CalculateWeights()
{
  IRTK_START_TIMING();
  ...
  IRTK_DEBUG_TIMING(6, "calculating correspondence weights (" << m << "x" << n << ")");

  // Initialize correspondence matrix
  IRTK_RESET_TIMING();
  _Weight.Initialize(m, n, entries, true);
  delete[] entries;
  IRTK_DEBUG_TIMING(7, "copying sparse matrix entries (NNZ=" << _Weight.NNZ() << ")");
}
