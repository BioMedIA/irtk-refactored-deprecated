...
#endif
  cout << endl;
  cout << "       Positional arguments have to be in front, arguments with preceeding option name" << endl;
  cout << "       can occur in any order. Repeated options may override previous option arguments." << endl;
}
...
void PrintUsage(const char* name)
{
  ...
  cout << "  -dofout <file>          Write transformation to specified file. (default: ireg.dof.gz)" << endl;
  ...
  cout << "                          the parameters are read from standard input instead. (default: ireg.cfg)" << endl;
...
void PrintHelp(const char* name)
{
  ...
  cout << "                               Alternatively, use \"-par 'Transformation model=<name>'\". (default: n/a)" << endl;
  cout << "  -dofout <file>               Write transformation to specified file. (default: ireg.dof.gz)" << endl;
  ...
  cout << "                               the preceeding image and/or polydata. Note the difference to -dofin," << endl;
  cout << "                               which specifies an initial estimate of the -dofout transformation." << endl;
  cout << "                               (default: none)" << endl;
  cout << "  -dof_i <file>                Specifies a pre-computed affine transformation whose inverse is to be" << endl;
  cout << "                               applied to the preceeding image and/or polydata (cf. -dof). (default: none)" << endl;
  ...
  cout << "                               as positional arguments or using the -image option, respectively." << endl;
  cout << "  -psets <file>                Equivalent to -images, but for point sets (-pset)." << endl;
  ...
  cout << "                               in the -images list file. (default: none)" << endl;
  cout << "  -dofin  <file>               Read initial transformation from file if -dofins not specified." << endl;
...
int read_input_list_file(const char *input_list_name, vector<string>& names, vector<double> &times)
{
...
int read_dofin_list_file(const char *list_name, const vector<string> &images, vector<int> &targets, vector<int> &sources, vector<string> &dofs)
{
...
private:
  ...
  vector<string>                         _DoFName;
  vector<bool>                           _DoFInvert;
  vector<vtkSmartPointer<vtkPointSet> > *_PointSet;
  Error                                 *_Error;
...
public:
  ...
  {
    double p[3];
    vector<vtkSmartPointer<vtkPointSet> > &pointset = *_PointSet;
    for (size_t n = re.begin(); n != re.end(); ++n) {
  ...
  static void Run(const vector<string>                  &fname,
                  const vector<string>                  &tname,
                  const vector<bool>                    &tinv,
                  vector<vtkSmartPointer<vtkPointSet> > &pointset,
                  Error                                 *error)
  {
...
int main(int argc, char **argv)
{
  ...
  vector<string> imdof_names;
  vector<bool>   imdof_invert;

  vector<string> pset_names;
  vector<string> pdof_names;
  vector<bool>   pdof_invert;
  ...
  const char *dofout_name        = "ireg.dof.gz";
...
#else
      cerr << "Point set input only supported when built with VTK!" << endl;
      cerr << "Rebuild this application after setting CMake option BUILD_WITH_VTK to ON." << endl;
      cerr << "Ensure also that the VTK_DIR points to a valid VTK installation." << endl;
      exit(1);
#endif
    }
  ...
  // ---------------------------------------------------------------------------
  // Read configuration
  irtkGenericRegistrationFilter registration;
...
#ifdef HAS_VTK
  vector<vtkSmartPointer<vtkPointSet> > psets(npsets);
  {
  ...
  IRTK_DEBUG_TIMING(1, "reading input data");

  // ---------------------------------------------------------------------------
  // Run registration
  irtkGenericRegistrationLogger   logger;
  irtkGenericRegistrationDebugger debugger("ireg_");
  ...
  irtkTransformation *dofout = NULL;
  ...
  if (verbose) {
    double elapsed_time;
#ifdef HAS_TBB
    elapsed_time = (tick_count::now() - start_time).seconds();
#else
  ...
  if (dofout_name && strcmp(dofout_name, "none") != 0 &&
                     ...
                     strcmp(dofout_name, "NONE") != 0) {
    if (dofout->TypeOfClass() == IRTKTRANSFORMATION_SIMILARITY) {
      ...
      // TODO: Update other tools (e.g., rview) to handle irtkSimilarityTransformation
      irtkAffineTransformation aff(*static_cast<irtkSimilarityTransformation *>(dofout));
