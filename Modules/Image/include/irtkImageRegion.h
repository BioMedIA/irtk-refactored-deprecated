...
#ifndef _irtkImageRegion_H

#define _irtkImageRegion_H
 ...
 * \code
 * irtkGreyImage   im(256, 256, 128);
 * irtkImageRegion it(image);
 *
 ...
 * \endcode
 *
 * \sa See irtkConstImageIterator, irtkImageIterator, irtkConstGenericImageIterator,
 *     and irtkGenericImageIterator for more advanced image iterators which do not
 ...
 *     the irtkGetMin class which can be used as Operation template argument
 *     of the ForEachVoxel functions. These template functions and basic
 *     voxel-wise operators are defined in the irtkVoxelFunction.h header file.
 */

class irtkImageRegion
{
public:
  ...
  /// Constructor
  irtkImageRegion(const irtkImageAttributes &);

  /// Constructor
  irtkImageRegion(const irtkImageAttributes &, const blocked_range2d<int> &);

  /// Constructor
  irtkImageRegion(const irtkImageAttributes &, const blocked_range3d<int> &);

  /// Constructor
  irtkImageRegion(const irtkBaseImage &);

  /// Constructor
  irtkImageRegion(const irtkBaseImage &, const blocked_range2d<int> &);

  /// Constructor
  irtkImageRegion(const irtkBaseImage &, const blocked_range3d<int> &);
  ...
  /// Copy constructor
  irtkImageRegion(const irtkImageRegion &);

  /// Assignment operator
  irtkImageRegion &operator =(const irtkImageRegion &);
  ...
  const VoxelType *GetPointerToBegin(const irtkBaseImage &) const;
  ...
  /// Get raw image pointer to first voxel of of region
  template <class VoxelType>
  const VoxelType *GetPointerToBegin(const irtkGenericImage<VoxelType> &) const;

  /// Get raw image pointer to first voxel of of region
  template <class VoxelType>
  VoxelType *GetPointerToBegin(irtkGenericImage<VoxelType> &) const;
  ...
  template <class VoxelType>
  const VoxelType *GetPointerToEnd(const irtkBaseImage &) const;
  ...
  /// Get raw image pointer to last voxel image of region
  template <class VoxelType>
  const VoxelType *GetPointerToEnd(const irtkGenericImage<VoxelType> &) const;

  /// Get raw image pointer to last voxel image of region
  template <class VoxelType>
  VoxelType *GetPointerToEnd(irtkGenericImage<VoxelType> &) const;
...
protected:
  ...
  // Region attributes
  irtkVector4D<int> _Index; ///< Start of set image region
  irtkVector4D<int> _Size;  ///< Size of  set image region
  irtkVector4D<int> _Begin; ///< First index of actual image region
  irtkVector4D<int> _End;   ///< Last  index of actual image region
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::Initialize()
{
  // Set iteration range
  _Begin = _Index;
  _End   = _Index + _Size;
...
// ---------------------------------------------------------------------------
inline bool irtkImageRegion::IsImageSequence() const
{
  return _IsImageSequence;
}

// ---------------------------------------------------------------------------
inline bool irtkImageRegion::IsScalarImage() const
{
  return _DataSize._t == 1;
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::NumberOfImageVoxels() const
{
  return _DataSize._x * _DataSize._y * _DataSize._z * _DataSize._t;
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::NumberOfImageChannels() const
{
  return _IsImageSequence ? 1 : _DataSize._t;
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::NumberOfVectorComponents() const
{
  return NumberOfImageChannels();
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::NumberOfSequenceFrames() const
{
...
// ---------------------------------------------------------------------------
inline bool irtkImageRegion::IsSequence() const
{
  return _Size._t > 1 && _IsImageSequence;
}

// ---------------------------------------------------------------------------
inline bool irtkImageRegion::IsScalar() const
{
  return _Size._t == 1;
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::MaxNumberOfVoxels() const
{
  return _Size._x * _Size._y * _Size._z * _Size._t;
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::MaxNumberOfChannels() const
{
  return IsSequence() ? 1 : _Size._t;
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::MaxNumberOfComponents() const
{
  return MaxNumberOfChannels();
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::MaxNumberOfFrames() const
{
  return IsSequence() ? _Size._t : 1;
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::NumberOfVoxels() const
{
  return (_End._x - _Begin._x) * (_End._y - _Begin._y) * (_End._z - _Begin._z) * (_End._t - _Begin._t);
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::NumberOfChannels() const
{
  return IsSequence() ? 1 : (_End._t - _Begin._t);
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::NumberOfComponents() const
{
  return NumberOfChannels();
}

// ---------------------------------------------------------------------------
inline int irtkImageRegion::NumberOfFrames() const
{
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetStart(int i, int j, int k, int l)
{
  _Index._x = i;
  _Index._y = j;
  _Index._z = k;
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetCenter(int i, int j, int k, int l)
{
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetSize(int s)
{
  _Size = s;
  _Begin._x = -1; // mark as invalid
}
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetSize(int ni, int nj, int nk, int nl)
{
  _Size._x = ni;
  _Size._y = nj;
  _Size._z = nk;
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetRadius(int r)
{
  _Size = 2 * r + 1;
  _Begin._x = -1; // mark as invalid
}
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetRadius(int ri, int rj, int rk, int rl)
{
  _Size._x = 2 * ri + 1;
  _Size._y = 2 * rj + 1;
  _Size._z = 2 * rk + 1;
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetRegion(int bi, int bj, int ei, int ej)
{
  _Index._x = bi;
  _Index._y = bj;
  _Index._z = 0;
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetRegion(const blocked_range2d<int> &r)
{
  SetRegion(r.cols().begin(), r.rows().begin(), r.cols().end(), r.rows().end());
}

// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetRegion(int bi, int bj, int bk, int ei, int ej, int ek)
{
  _Index._x = bi;
  _Index._y = bj;
  _Index._z = bk;
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetRegion(int bi, int bj, int bk, int bl, int ei, int ej, int ek, int el)
{
  _Index._x = bi;
  _Index._y = bj;
  _Index._z = bk;
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetRegion(const blocked_range3d<int> &r)
{
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetChannel(int l)
{
  if (!_IsImageSequence) {
    if (l < 0 || l >= _DataSize._t) {
      cerr << "irtkImageRegion::SetChannel: Index out of bounds" << endl;
      exit(1);
    }
    _Index._t = l;
    _Begin._x = -1; // mark as invalid
  } else if (l != 0) {
    cerr << "irtkImageRegion::SetChannel: Index out of bounds (an image sequence can only have one channel)" << endl;
    exit(1);
  }
}

// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetChannel(int bl, int el)
{
  if (!_IsImageSequence) {
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetChannel(const blocked_range<int> &r)
{
  SetChannel(r.begin(), r.end());
}

// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetComponent(int l)
{
  if (!_IsImageSequence) {
    if (l < 0 || l >= _DataSize._t) {
      cerr << "irtkImageRegion::SetComponent: Index out of bounds" << endl;
      exit(1);
    }
    _Index._t = l;
    _Begin._x = -1; // mark as invalid
  } else if (l != 0) {
    cerr << "irtkImageRegion::SetComponent: Index out of bounds (an image sequence can only have one component)" << endl;
    exit(1);
  }
}

// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetComponent(int bl, int el)
{
  if (!_IsImageSequence) {
    if (bl < 0 || bl >= _DataSize._t || el < 0 || el >= _DataSize._t) {
      cerr << "irtkImageRegion::SetComponent: Index out of bounds" << endl;
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetComponent(const blocked_range<int> &r)
{
  SetComponent(r.begin(), r.end());
}

// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetFrame(int l)
{
  if (_IsImageSequence) {
    if (l < 0 || l >= _DataSize._t) {
      cerr << "irtkImageRegion::SetFrame: Index out of bounds" << endl;
      exit(1);
    }
    _Index._t = l;
    _Begin._x = -1; // mark as invalid
  } else if (l != 0) {
    cerr << "irtkImageRegion::SetFrame: Index out of bounds (image seems to have multiple channels/vector components instead)" << endl;
    exit(1);
  }
}

// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetFrame(int bl, int el)
{
  if (_IsImageSequence) {
    if (bl < 0 || bl >= _DataSize._t || el < 0 || el >= _DataSize._t) {
      cerr << "irtkImageRegion::SetFrame: Index out of bounds" << endl;
...
// ---------------------------------------------------------------------------
inline void irtkImageRegion::SetFrame(const blocked_range<int> &r)
{
...
// ---------------------------------------------------------------------------
inline irtkImageRegion::irtkImageRegion(const irtkImageAttributes &attr)
:
...
// ---------------------------------------------------------------------------
inline irtkImageRegion::irtkImageRegion(const irtkImageAttributes &attr, const blocked_range2d<int> &region)
:
  _DataSize       (attr._x, attr._y, attr._z, attr._t),
  _IsImageSequence(attr._t > 1 && attr._dt > .0),
  _Index          (0, 0, 0, 0),
...
// ---------------------------------------------------------------------------
inline irtkImageRegion::irtkImageRegion(const irtkImageAttributes &attr, const blocked_range3d<int> &region)
:
  _DataSize       (attr._x, attr._y, attr._z, attr._t),
  _IsImageSequence(attr._t > 1 && attr._dt > .0),
  _Index          (0, 0, 0, 0),
...
// ---------------------------------------------------------------------------
inline irtkImageRegion::irtkImageRegion(const irtkBaseImage &image)
:
...
// ---------------------------------------------------------------------------
inline irtkImageRegion::irtkImageRegion(const irtkBaseImage &image, const blocked_range2d<int> &region)
:
  _DataSize       (image.GetX(), image.GetY(), image.GetZ(), image.GetT()),
  _IsImageSequence(image.GetT() > 1 && image.GetTSize() > .0),
  _Index          (0, 0, 0, 0),
...
// ---------------------------------------------------------------------------
inline irtkImageRegion::irtkImageRegion(const irtkBaseImage &image, const blocked_range3d<int> &region)
:
  _DataSize       (image.GetX(), image.GetY(), image.GetZ(), image.GetT()),
  _IsImageSequence(image.GetT() > 1 && image.GetTSize() > .0),
  _Index          (0, 0, 0, 0),
...
// ---------------------------------------------------------------------------
inline irtkImageRegion::irtkImageRegion(const irtkImageRegion &other)
:
...
// ---------------------------------------------------------------------------
inline irtkImageRegion &irtkImageRegion::operator =(const irtkImageRegion &rhs)
{
...
// ---------------------------------------------------------------------------
inline irtkImageRegion::~irtkImageRegion()
{
}
...
// -----------------------------------------------------------------------------
template <class VoxelType>
inline const VoxelType *irtkImageRegion::GetPointerToBegin(const irtkBaseImage &image) const
{
  if (image.GetScalarType() != voxel_info<VoxelType>::type()) {
    cerr << "irtkImageRegion::GetPointerToBegin: Type of image differs from VoxelType template argument" << endl;
    exit(1);
  }
  if (_Begin._x < 0) const_cast<irtkImageRegion *>(this)->Initialize();
...
// -----------------------------------------------------------------------------
template <class VoxelType>
inline const VoxelType *irtkImageRegion::GetPointerToBegin(const irtkGenericImage<VoxelType> &image) const
{
  if (_Begin._x < 0) const_cast<irtkImageRegion *>(this)->Initialize();
...
// -----------------------------------------------------------------------------
template <class VoxelType>
inline VoxelType *irtkImageRegion::GetPointerToBegin(irtkGenericImage<VoxelType> &image) const
{
  if (_Begin._x < 0) const_cast<irtkImageRegion *>(this)->Initialize();
  return image.GetPointerToVoxels(_Begin._x, _Begin._y, _Begin._z, _Begin._t);
}
...
// -----------------------------------------------------------------------------
template <class VoxelType>
inline const VoxelType *irtkImageRegion::GetPointerToEnd(const irtkBaseImage &image) const
{
  if (image.GetScalarType() != voxel_info<VoxelType>::type()) {
    cerr << "irtkImageRegion::GetPointerToEnd: Type of image differs from VoxelType template argument" << endl;
    exit(1);
  }
  if (_Begin._x < 0) const_cast<irtkImageRegion *>(this)->Initialize();
...
// -----------------------------------------------------------------------------
template <class VoxelType>
inline const VoxelType *irtkImageRegion::GetPointerToEnd(const irtkGenericImage<VoxelType> &image) const
{
  if (_Begin._x < 0) const_cast<irtkImageRegion *>(this)->Initialize();
...
// -----------------------------------------------------------------------------
template <class VoxelType>
inline VoxelType *irtkImageRegion::GetPointerToEnd(irtkGenericImage<VoxelType> &image) const
{
  if (_Begin._x < 0) const_cast<irtkImageRegion *>(this)->Initialize();
  return image.GetPointerToVoxels(_End._x - 1, _End._y - 1, _End._z - 1, _End._t - 1);
}
