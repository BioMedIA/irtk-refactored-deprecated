...
#ifndef _IRTKPOLYDATACURVATURE_H
#define _IRTKPOLYDATACURVATURE_H
 ...
 */
class irtkPolyDataCurvature : public irtkPolyDataFilter
{
  irtkObjectMacro(irtkPolyDataCurvature);
...
protected:
  ...
  irtkPublicAttributeMacro(int, CurvatureType);

  /// Whether to use vtkCurvatures filter instead
  irtkPublicAttributeMacro(bool, VtkCurvatures);

  /// Number of tensor averaging iterations
  irtkPublicAttributeMacro(int, TensorAveraging);

  /// Whether to normalize curvature assuming a spherical shape
  irtkPublicAttributeMacro(bool, Normalize);

  /// Volume of convex hull
  /// \note Compute only if NormalizeOn.
  irtkReadOnlyAttributeMacro(double, Volume);

  /// Radius of sphere have the same volume as the convex hull
  /// \note Compute only if NormalizeOn.
  irtkReadOnlyAttributeMacro(double, Radius);

  /// Copy attributes of this class from another instance
  void Copy(const irtkPolyDataCurvature &);
...
public:
  ...
  /// Default constructor
  irtkPolyDataCurvature();

  /// Copy constructor
  irtkPolyDataCurvature(const irtkPolyDataCurvature &);

  /// Assignment operator
  irtkPolyDataCurvature &operator =(const irtkPolyDataCurvature &);

  /// Destructor
  virtual ~irtkPolyDataCurvature();
