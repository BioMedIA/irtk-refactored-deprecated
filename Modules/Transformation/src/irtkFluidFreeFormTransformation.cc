...
// -----------------------------------------------------------------------------
irtkFluidFreeFormTransformation::irtkFluidFreeFormTransformation()
{
}

// -----------------------------------------------------------------------------
irtkFluidFreeFormTransformation::irtkFluidFreeFormTransformation(const irtkRigidTransformation &t)
:
...
// -----------------------------------------------------------------------------
irtkFluidFreeFormTransformation::irtkFluidFreeFormTransformation(const irtkAffineTransformation &t)
:
  irtkMultiLevelTransformation(t)
{
}
...
// -----------------------------------------------------------------------------
irtkFluidFreeFormTransformation::irtkFluidFreeFormTransformation(const irtkFluidFreeFormTransformation &t)
:
  irtkMultiLevelTransformation(t)
{
}
...
// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation::CheckTransformation(irtkFreeFormTransformation *transformation) const
{
  irtkMultiLevelTransformation::CheckTransformation(transformation);
}
...
// -----------------------------------------------------------------------------
double irtkFluidFreeFormTransformation
...
// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation
...
// -----------------------------------------------------------------------------
bool irtkFluidFreeFormTransformation::CopyFrom(const irtkTransformation *other)
{
  bool ok = irtkMultiLevelTransformation::CopyFrom(other);
  const irtkFluidFreeFormTransformation *fluid;
  if (ok && (fluid = dynamic_cast<const irtkFluidFreeFormTransformation *>(other))) {
...
// -----------------------------------------------------------------------------
bool irtkFluidFreeFormTransformation::IsIdentity() const
{
  return _AffineTransformation.IsIdentity() && irtkMultiLevelTransformation::IsIdentity();
}

// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation::Reset()
{
  irtkMultiLevelTransformation::Reset();
  _AffineTransformation.Reset();
}

// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation::Clear()
{
  irtkMultiLevelTransformation::Clear();
  _AffineTransformation.Reset();
}
...
// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation::Transform(int m, int n, double &x, double &y, double &z, double, double) const
{
...
// -----------------------------------------------------------------------------
bool irtkFluidFreeFormTransformation::Inverse(int m, int n, double &x, double &y, double &z, double, double) const
{
  bool ok = true;
...
// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation::Displacement(int m, int n, irtkGenericImage<double> &disp, double t, double t0, const irtkWorldCoordsImage *wc) const
{
  ...
  if (disp.GetT() < 2 || disp.GetT() > 3) {
    cerr << "irtkFluidFreeFormTransformation::Displacement: Input/output image must have either 2 or 3 vector components (_t)" << endl;
    exit(1);
  }
...
// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation::Displacement(int m, int n, irtkGenericImage<float> &disp, double t, double t0, const irtkWorldCoordsImage *wc) const
{
  // Call base class function if caching not required
  if (!this->RequiresCachingOfDisplacements()) {
    irtkMultiLevelTransformation::Displacement(m, n, disp, t, t0, wc);
...
// -----------------------------------------------------------------------------
int irtkFluidFreeFormTransformation::InverseDisplacement(int m, int n, irtkGenericImage<double> &disp, double t, double t0, const irtkWorldCoordsImage *wc) const
{
  // Call base class function if caching not required
  if (!this->RequiresCachingOfDisplacements()) {
  ...
  // Check arguments
  if (disp.GetT() < 2 || disp.GetT() > 3) {
    cerr << "irtkFluidFreeFormTransformation::InverseDisplacement: Input/output image must have either 2 or 3 vector components (_t)" << endl;
    exit(1);
  }
...
// -----------------------------------------------------------------------------
int irtkFluidFreeFormTransformation::InverseDisplacement(int m, int n, irtkGenericImage<float> &disp, double t, double t0, const irtkWorldCoordsImage *wc) const
{
  // Call base class function if caching not required
  if (!this->RequiresCachingOfDisplacements()) {
    return irtkMultiLevelTransformation::InverseDisplacement(m, n, disp, t, t0, wc);
...
// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation::Jacobian(int m, int n, irtkMatrix &jac, double x, double y, double z, double, double) const
{
...
// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation::Hessian(int m, int n, irtkMatrix hessian[3], double x, double y, double z, double, double) const
{
  // Initialize matrices
  ...
  irtkMatrix localjac, localhessian[3];
                     ...
                     double t0, double w) const
{
  ...
  for (;;) {
    ...
    const irtkFreeFormTransformation *ffd = this->GetLocalTransformation(active);
...
// -----------------------------------------------------------------------------
void irtkFluidFreeFormTransformation::Print(irtkIndent indent) const
{
...
// -----------------------------------------------------------------------------
bool irtkFluidFreeFormTransformation::CanRead(irtkTransformationType format) const
{
  switch (format) {
...
// -----------------------------------------------------------------------------
irtkCifstream &irtkFluidFreeFormTransformation::ReadDOFs(irtkCifstream &from, irtkTransformationType format)
{
  // Read global and local transformations
  irtkMultiLevelTransformation::ReadDOFs(from, format);
...
// -----------------------------------------------------------------------------
irtkCofstream &irtkFluidFreeFormTransformation::WriteDOFs(irtkCofstream &to) const
{
  // Write global and local transformations
  irtkMultiLevelTransformation::WriteDOFs(to);
