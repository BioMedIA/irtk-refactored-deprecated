...
#ifndef _IRTKDATAFUNCTIONS_H
#define _IRTKDATAFUNCTIONS_H

#include <irtkDataOp.h>
#include <irtkDataStatistics.h>


namespace irtk { namespace data { namespace op {
...
class ElementWiseBinaryOp : public Op
{
  ...
  irtkPublicAttributeMacro(double, Constant);
...
{
  irtkPublicAttributeMacro(double, Base);
  irtkPublicAttributeMacro(double, Threshold);
...
public:
  ...
  {
    if (_FileName.empty()) mask = !fequal(value, constant);
    else                   mask = static_cast<bool>(constant);
    return value;
  }
...
// -----------------------------------------------------------------------------
/// Mask below or above a specified interval
class MaskInterval : public ElementWiseUnaryOp
{
  /// Lower threshold value
  irtkPublicAttributeMacro(double, LowerThreshold);

  /// Upper threshold value
  irtkPublicAttributeMacro(double, UpperThreshold);

public:

  /// Constructor
  MaskInterval(double l, double u) : _LowerThreshold(l), _UpperThreshold(u) {}

  /// Transform data value and/or mask data value by setting *mask = false
  virtual double Op(double value, bool &mask) const
  {
    mask = (_LowerThreshold < value && value < _UpperThreshold);
    return value;
  }
...
public:
  ...
  virtual double Op(double value, bool &mask) const
  {
    mask = (static_cast<int>(value) % 2 == 0);
    return value;
  }
...
public:
  ...
  virtual double Op(double value, bool &mask) const
  {
    mask = (static_cast<int>(value) % 2 == 1);
    return value;
  }
...
// -----------------------------------------------------------------------------
/// Mask values below or equal a given threshold value
