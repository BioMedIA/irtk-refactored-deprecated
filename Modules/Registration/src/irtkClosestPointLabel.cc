...
namespace irtkClosestPointLabelUtils {


// -----------------------------------------------------------------------------
/// Determine number of distinct labels
int NumberOfLabels(vtkDataArray *labels, vector<int> &labelSet, map<int, int> &labelPos)
{
  set<int> uniqueLabelSet;
  ...
  labelPos.clear();
  for (set<int>::const_iterator i = uniqueLabelSet.begin(); i != uniqueLabelSet.end(); ++i, ++idx) {
...
struct FindClosestLabeledPoints
{
  ...
  const map<int, int>     *_LabelIndex;
  vtkDataArray            *_ClosestPoint;
  vector<int>             *_SourceIndex;
...
{
  vtkSmartPointer<vtkDataArray> pointLabels = cellLabels->NewInstance();
...
// -----------------------------------------------------------------------------
void UpdateClosestLabeledPoints(const irtkRegisteredPointSet    *target,
                                const irtkRegisteredPointSet    *source,
                                vtkSmartPointer<vtkIdTypeArray> &closest_points,
                                map<int, int>                   &label_pos)
{
  ...
  if (targetLabels == NULL || sourceLabels == NULL) {
    cerr << "irtkClosestPointLabel::Update: Missing data array named \"Labels\" (case insensitive)" << endl;
    exit(1);
  }
  vector<int> labelSet;
  ...
  IRTK_START_TIMING();
  irtkEdgeTable sourceEdges(source->PointSet());
  IRTK_DEBUG_TIMING(5, "building the edge table");
  ...
  IRTK_DEBUG_TIMING(5, "distance transform for each label");
}

// -----------------------------------------------------------------------------
/// Find corresponding source points with label identical to the one of the target point
void UpdateCorrespondences(const irtkRegisteredPointSet *target,
                           const irtkRegisteredPointSet *source,
                           vtkIdTypeArray               *closest_points,
                           const map<int, int>          &label_pos,
                           vector<int>                  &cor,
                           vector<double>               &distance)
{
  ...
  IRTK_DEBUG_TIMING(5, "update of point correspondences");
}


} // namespace irtkClosestPointLabelUtils
using namespace irtkClosestPointLabelUtils;
...
// -----------------------------------------------------------------------------
irtkClosestPointLabel::irtkClosestPointLabel()
{
}

// -----------------------------------------------------------------------------
irtkClosestPointLabel::irtkClosestPointLabel(const irtkClosestPointLabel &other)
:
  irtkClosestPoint(other),
...
// -----------------------------------------------------------------------------
irtkPointCorrespondence *irtkClosestPointLabel::NewInstance() const
{
  return new irtkClosestPointLabel(*this);
}
...
// -----------------------------------------------------------------------------
irtkClosestPointLabel::TypeId irtkClosestPointLabel::Type() const
{
  return ClosestPointLabel;
}
...
// -----------------------------------------------------------------------------
void irtkClosestPointLabel::Init()
{
...
// -----------------------------------------------------------------------------
void irtkClosestPointLabel::Initialize()
{
  // Initialize base class
  irtkClosestPoint::Initialize();

  // Initialize this class
  irtkClosestPointLabel::Init();
}

// -----------------------------------------------------------------------------
void irtkClosestPointLabel::Reinitialize()
{
  // Reinitialize base class
  irtkClosestPoint::Reinitialize();
...
// -----------------------------------------------------------------------------
void irtkClosestPointLabel::Update()
{
