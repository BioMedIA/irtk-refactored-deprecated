...
namespace irtkTransformationApproximationErrorUtils {
...
struct Evaluate
{
  ...
  void operator ()(const blocked_range<int> &re)
  {
    ...
    for (int n = re.begin(); n != re.end(); ++n) {
      const irtkPoint &p1 = _Target->GetPoint(n);
      const irtkPoint &p2 = _Source->GetPoint(n);
      dx = p1._x - p2._x;
      dy = p1._y - p2._y;
      dz = p1._z - p2._z;
...
// -----------------------------------------------------------------------------
irtkTransformationApproximationError
::irtkTransformationApproximationError(irtkTransformation *transformation,
  ...
  _Gradient(NULL)
{
  ...
  int max_no = 0, cur_no;
  set<double>::const_iterator time = times.begin();
  for (int l = 0; l < _NumberOfTimePoints; ++l, ++time) {
    ...
    for (int n = 0; n < no; ++n) {
      if (fequal(t[n], _TargetTime[l])) {
        _Target[l].Add(irtkPoint(x[n],         y[n],         z[n]));
        _Source[l].Add(irtkPoint(x[n] + dx[n], y[n] + dy[n], z[n] + dz[n]));
      }
    }
  }
...
// -----------------------------------------------------------------------------
irtkTransformationApproximationError::~irtkTransformationApproximationError()
{
  Deallocate(_Gradient);
}

// -----------------------------------------------------------------------------
void irtkTransformationApproximationError::CenterPoints()
{
  _TargetCenter = _SourceCenter = irtkPoint(.0, .0, .0);
...
// -----------------------------------------------------------------------------
int irtkTransformationApproximationError::NumberOfDOFs() const
{
  return _Transformation->NumberOfDOFs();
}

// -----------------------------------------------------------------------------
void irtkTransformationApproximationError::Put(const double *x)
{
  _Transformation->Put(x);
}

// -----------------------------------------------------------------------------
double irtkTransformationApproximationError::Get(int i) const
{
  return _Transformation->Get(i);
}

// -----------------------------------------------------------------------------
void irtkTransformationApproximationError::Get(double *x) const
{
  _Transformation->Get(x);
}

// -----------------------------------------------------------------------------
double irtkTransformationApproximationError::Step(const double *dx)
{
  return _Transformation->Update(dx);
}

// -----------------------------------------------------------------------------
void irtkTransformationApproximationError::Update(bool)
{
...
// -----------------------------------------------------------------------------
double irtkTransformationApproximationError::Value()
{
  double sum = .0;
  int    num =  0;
  for (int l = 0; l < _NumberOfTimePoints; ++l) {
    irtkTransformationApproximationErrorUtils::Evaluate eval;
...
// -----------------------------------------------------------------------------
void irtkTransformationApproximationError::Gradient(double *dx, double, bool *)
{
  memset(dx, 0, this->NumberOfDOFs() * sizeof(double));
  irtkTransformationApproximationErrorUtils::EvaluateGradient eval;
...
// -----------------------------------------------------------------------------
double irtkTransformationApproximationError::GradientNorm(const double *dx) const
{
