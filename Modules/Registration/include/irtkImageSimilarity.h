...
#ifndef _IRTKIMAGESIMILARITY_H
#define _IRTKIMAGESIMILARITY_H
 ...
 */
class irtkImageSimilarity : public irtkDataFidelity
{
  irtkAbstractMacro(irtkImageSimilarity);
...
public:
  ...
  typedef irtkRegisteredImage::VoxelType           VoxelType;

  /// Type of similarity gradient image
  typedef irtkRegisteredImage::GradientImageType   GradientImageType;
  ...
  irtkPublicComponentMacro(irtkRegisteredImage, Target);

  /// (Transformed) Source image
  irtkPublicComponentMacro(irtkRegisteredImage, Source);
  ...
  irtkPublicAttributeMacro(bool, UseApproximateGradient);
  ...
  irtkPublicAttributeMacro(double, VoxelWisePreconditioning);
  ...
  /// Zikic, D., Baust, M., Kamen, A., & Navab, N. A General Preconditioning
  /// Scheme for Difference Measures in Deformable Registration. In ICCV 2011.
  irtkPublicAttributeMacro(double, NodeBasedPreconditioning);
...
protected:
  ...
  /// Constructor
  irtkImageSimilarity(const char * = "", double = 1.0);

  /// Copy constructor
  irtkImageSimilarity(const irtkImageSimilarity &);

  /// Assignment operator
  irtkImageSimilarity &operator =(const irtkImageSimilarity &);

public:

  /// Instantiate specified similarity measure
  static irtkImageSimilarity *New(irtkSimilarityMeasure);

  /// Destructor
  virtual ~irtkImageSimilarity();
...
public:
  ...
  using irtkDataFidelity::Parameter;
...
protected:
  ...
  /// \frac{dSimilarity}{dy} = \frac{dSimilarity}{dI} * \frac{dI}{dy}, given
  /// \frac{dSimilarity}{dI} as input, where y = T(x).
  ///
  ...
  void MultiplyByImageGradient(const irtkRegisteredImage *image,
  ...
  virtual bool NonParametricGradient(const irtkRegisteredImage *image,
                                     GradientImageType         *gradient);
  ...
  /// \param[in]     weight   Weight of image similarity.
  void ApproximateGradient(irtkRegisteredImage        *image,
  ...
  virtual void NormalizeGradient(const irtkRegisteredImage *image, double *gradient);
  ...
  /// \param[in]     step        Step size to use for finite differences.
  /// \param[in]     weight      Weight of image similarity.
  virtual void EvaluateGradient(irtkRegisteredImage  *image,
                                GradientImageType   *&np_gradient,
                                double               *gradient,
...
// -----------------------------------------------------------------------------
inline bool irtkImageSimilarity::IsForeground(int idx) const
{
...
// -----------------------------------------------------------------------------
inline bool irtkImageSimilarity::IsForeground(int i, int j, int k) const
{
  // Never evaluate similarity outside explicitly specified domain
