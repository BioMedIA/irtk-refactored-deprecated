...
#ifndef _IRTKMULTIPLEVOXELTRANSFORMATION_H
#define _IRTKMULTIPLEVOXELTRANSFORMATION_H

#include <irtkVoxelFunction.h>


/**
 * These voxel functions are used by irtkMultipleImageTransformation
 * to efficiently transform one or more input images at once
 *
 */

namespace irtkMultipleVoxelTransformation {
...
// -----------------------------------------------------------------------------
/// Base class for voxel transformation functions
class Base : public irtkVoxelFunction
{
protected:
...
struct TransformData
{
  ...
  const irtkImage              **_Inputs;                ///< Input images
  const irtkImage               *_Input;                 ///< Single/reference input
  irtkInterpolateImageFunction **_Interpolators;         ///< Input interpolators
  irtkInterpolateImageFunction  *_Interpolator;          ///< Single/reference interpolator
  double                        *_PaddingValues;         ///< Outside padding values
  double                         _PaddingValue;          ///< Default padding value
  const irtkTransformation      *_Transformation1;       ///< First transformation
  const irtkTransformation      *_Transformation2;       ///< Second transformation (optional)
  ...
  irtkImage                    **_Outputs;               ///< Output images
  irtkImage                     *_Output;                ///< Single/reference output image
  irtkBinaryImage               *_Mask;                  ///< Output foreground mask
 ...
 * instantiating this class, use the default irtkInterpolateImageFunction base
 * interpolate image function type.
 *
 */
template <class InterpolateImageFunction = irtkInterpolateImageFunction,
          class InputDomain              = irtkInterpolationDomain::Foreground>
    ...
    TransformData(data), Base()
  {
    ...
    if (!_Inputs || !_Interpolators || !_Outputs) {
      if (_NumberOfImages > 1) {
        cerr << "irtkMultipleVoxelTransformation::BaseTransform: Invalid TransformData" << endl;
        exit(1);
      }
      if (!_Inputs) {
        if (!_Input) {
          cerr << "irtkMultipleVoxelTransformation::BaseTransform: Missing input image" << endl;
          exit(1);
        }
        _Inputs = &_Input;
      }
      if (!_Interpolators) {
        if (!_Interpolator) {
          cerr << "irtkMultipleVoxelTransformation::BaseTransform: Missing input interpolator" << endl;
          exit(1);
        }
      ...
      if (!_Outputs) {
        if (!_Output) {
          cerr << "irtkMultipleVoxelTransformation::BaseTransform: Missing output image" << endl;
          exit(1);
        }
        _Outputs = &_Output;
      }
...
// -----------------------------------------------------------------------------
template <class InterpolateImageFunction = irtkInterpolateImageFunction, class InputDomain = irtkInterpolationDomain::Foreground>
