...
struct irtkIteration
{
  ...
  irtkIteration() : _Iter(0), _Begin(0), _End(1), _Inc(1) {}

  irtkIteration(int b, int e, int s = 1)
    : _Iter(b), _Begin(b), _End(e), _Inc((_End >= _Begin ? 1 : -1) * abs(s))
  {
    if (_Inc == 0) {
      cerr << "irtkIteration::irtkIteration: Invalid increment value" << endl;
      exit(1);
    }
  }
