...
#include <irtkClosestPointLabel.h>
#include <irtkClosestCell.h>
#include <irtkSpectralMatch.h>
#include <irtkRobustClosestPoint.h>
#include <irtkRobustPointMatch.h>

// -----------------------------------------------------------------------------
irtkPointCorrespondence *irtkPointCorrespondence::New(TypeId type)
{
  switch (type) {
    case FiducialMatch:       return new irtkFiducialMatch();
    case ClosestPoint:        return new irtkClosestPoint();
    case ClosestPointLabel:   return new irtkClosestPointLabel();
    case ClosestCell:         return new irtkClosestCell();
    case SpectralMatch:       return new irtkSpectralMatch();
    case RobustClosestPoint:  return new irtkRobustClosestPoint();
    case RobustPointMatch:    return new irtkRobustPointMatch();
    default:
      cerr << "irtkPointCorrespondence::New: Unknown type = " << type << endl;
      exit(1);
  }
}

// -----------------------------------------------------------------------------
irtkPointCorrespondence *irtkPointCorrespondence::New(const char *type_name)
{
  TypeId type = Unknown;
  if (!FromString(type_name, type)) {
    cerr << "irtkPointCorrespondence::New: Unknown type = " << type_name << endl;
    exit(1);
  }
  return New(type);
}
...
// -----------------------------------------------------------------------------
template <> bool FromString(const char *str, irtkPointCorrespondence::TypeId &type)
{
  if (strcmp(str, "Index") == 0 || strcmp(str, "Fiducial") == 0) {
    type = irtkPointCorrespondence::FiducialMatch;
  } else if (strcmp(str, "CP")            == 0 ||
             strcmp(str, "ClosestPoint")  == 0 ||
             strcmp(str, "Closest Point") == 0 ||
             strcmp(str, "Closest point") == 0) {
    type = irtkPointCorrespondence::ClosestPoint;
  } else if (strcmp(str, "ClosestPointLabel")   == 0 ||
             strcmp(str, "Closest Point Label") == 0 ||
             strcmp(str, "Closest point label") == 0) {
    type = irtkPointCorrespondence::ClosestPointLabel;
    ...
    type = irtkPointCorrespondence::ClosestCell;
  } else if (strcmp(str, "SM")                   == 0 ||
             strcmp(str, "SpectralMatch")   == 0 ||
             strcmp(str, "Spectral Match") == 0 ||
             strcmp(str, "Spectral match") == 0) {
    type = irtkPointCorrespondence::SpectralMatch;
  } else if (strcmp(str, "RCP")                  == 0 ||
             strcmp(str, "RobustClosestPoint")   == 0 ||
             strcmp(str, "Robust Closest Point") == 0 ||
             strcmp(str, "Robust closest point") == 0) {
    type = irtkPointCorrespondence::RobustClosestPoint;
  } else if (strcmp(str, "RPM")                == 0 ||
             strcmp(str, "RobustPointMatch")   == 0 ||
             strcmp(str, "Robust Point Match") == 0 ||
             strcmp(str, "Robust point match") == 0) {
    type = irtkPointCorrespondence::RobustPointMatch;
  } else {
    type = irtkPointCorrespondence::Unknown;
  }
  return (type != irtkPointCorrespondence::Unknown);
}

// -----------------------------------------------------------------------------
template <> string ToString(const irtkPointCorrespondence::TypeId &type, int w, char c, bool left)
{
  string str;
  switch (type) {
    case irtkPointCorrespondence::FiducialMatch:      str = "Index";              break;
    case irtkPointCorrespondence::ClosestPoint:       str = "ClosestPoint";       break;
    case irtkPointCorrespondence::ClosestPointLabel:  str = "ClosestPointLabel";
    case irtkPointCorrespondence::ClosestCell:        str = "ClosestCell";        break;
    case irtkPointCorrespondence::SpectralMatch:      str = "SpectralMatch";      break;
    case irtkPointCorrespondence::RobustClosestPoint: str = "RobustClosestPoint"; break;
    case irtkPointCorrespondence::RobustPointMatch:   str = "RobustPointMatch";   break;
...
public:
  ...
  static void Run(vtkAbstractPointLocator *locator, vtkPointSet *samples, vector<int> &index)
  {
...
// -----------------------------------------------------------------------------
void SamplePoints(vtkPointSet *pointset, vector<int> &indices,
                  int maxnum, double maxdist, bool stratified)
{
  ...
  if (maxnum == 0 && maxdist > .0) {
    ...
    nsamples = round(pointset->GetNumberOfPoints() / (sum / pointset->GetNumberOfPoints()));
  } else {
    nsamples = maxnum;
  }
  ...
  if (nsamples > 0) {
    ...
    IRTK_START_TIMING();
    sampler->Update();
    IRTK_DEBUG_TIMING(6, "uniformly subsampling point set");
    IRTK_RESET_TIMING();
    FindIndicesOfPointsClosestToSamples::Run(locator, sampler->GetOutput(), indices);
    IRTK_DEBUG_TIMING(6, "finding closest sample points");
  }

  IRTK_DEBUG_TIMING(5, "subsampling of point set (#samples = " << nsamples << ")");
}
...
vtkSmartPointer<vtkPointSet> SpectralPoints(vtkPointSet *input)
{
  ...
  vtkSmartPointer<vtkPointSet> output = input->NewInstance();
...
// -----------------------------------------------------------------------------
int GetEigenmodes(const irtkRegisteredPointSet *dataset, int k, irtkMatrix &m, irtkVector &v)
{
  int           i = -2;
  ...
  if (dataset->Transformation() || s == NULL || s->GetNumberOfComponents() < k || v.Rows() < k) {
    ...
    if (!polydata) {
      cerr << "irtkPointCorrespondence: Can only compute eigenmodes of surface mesh" << endl;
      exit(1);
    }
    if (irtkSpectralDecomposition::ComputeEigenmodes(polydata, k, m, v) != k) {
      cerr << "irtkPointCorrespondence: Failed to compute " << k << " eigenmodes" << endl;
      exit(1);
    }
    return -2;
  } else {
...
void UpdateEigenmodes(const irtkRegisteredPointSet         *target,
                      irtkVector                           &target_eigenvalues,
                      irtkPointCorrespondence::FeatureList &target_features,
                      const irtkRegisteredPointSet         *source,
                      irtkVector                           &source_eigenvalues,
                      irtkPointCorrespondence::FeatureList &source_features,
                      int                                   k)
{
  using namespace irtkSpectralDecomposition;
  // Get feature entries
  irtkPointCorrespondence::FeatureList::iterator target_info;
  ...
  irtkPointCorrespondence::FeatureList::iterator source_info;
  ...
  if (!target_polydata || !source_polydata) {
    cerr << "irtkFiducialRegistrationError::UpdateEigenmodes: Point sets must be surface meshes" << endl;
    exit(1);
  }
...
void UpdateEigenmodes(const irtkRegisteredPointSet         *target,
                      ...
                      const irtkRegisteredPointSet         *source,
                      irtkPointCorrespondence::FeatureList &source_features,
                      irtkVector                           &eigenvalues,
                      int                                  k)
{
  using namespace irtkSpectralDecomposition;
  typedef irtkGenericSparseMatrix<double> SparseMatrix;
  ...
  if (ComputeEigenmodes(target_polydata, source_polydata, k) < k) {
    cerr << "irtkFiducialRegistrationError::UpdateEigenmodes: Failed to find " << k << " initial eigenmodes" << endl;
    exit(1);
  }
  // Sample points for which inter-dataset links will be added
  vector<int> target_sample, source_sample;
  ...
  irtkPointLocator::FeatureList target_match_features;
  irtkPointLocator::FeatureList source_match_features;
  irtkPointLocator::FeatureInfo feature;
  ...
  corr12 = irtkPointLocator::FindClosestPoint(target->PointSet(), &target_sample, &target_match_features,
                                              source->PointSet(), &source_sample, &source_match_features, &dist12);
  corr21 = irtkPointLocator::FindClosestPoint(source->PointSet(), &source_sample, &source_match_features,
  ...
  for (size_t i = 0; i < corr12.size(); ++i) {
    ...
    const int c = irtkPointCorrespondence::GetPointIndex(target, &target_sample, i);
    cols[c].push_back(make_pair(r, dist12[i]));
    cols[r].push_back(make_pair(c, dist12[i]));
  }
  for (size_t i = 0; i < corr21.size(); ++i) {
    dist21[i] = 1.0 / (sqrt(dist21[i]) + EPSILON);
    const int r = corr21[i];
    const int c = irtkPointCorrespondence::GetPointIndex(source, &source_sample, i) + m;
    cols[c].push_back(make_pair(r, dist21[i]));
    cols[r].push_back(make_pair(c, dist21[i]));
  }
  ...
  irtkMatrix eigenmodes;
  if (ComputeEigenmodes(L, k+2, eigenmodes, eigenvalues) < k) {
    cerr << "irtkFiducialRegistrationError::UpdateEigenmodes: Failed to compute " << k << " eigenmodes" << endl;
    exit(1);
  }
...
// -----------------------------------------------------------------------------
void UpdateEigenmodesIfUsed(const irtkRegisteredPointSet         *target,
                            irtkVector                           &target_eigenvalues,
                            irtkPointCorrespondence::FeatureList &target_features,
                            const irtkRegisteredPointSet         *source,
                            irtkVector                           &source_eigenvalues,
...
using namespace irtkPointCorrespondenceUtils;
...
// -----------------------------------------------------------------------------
irtkPointCorrespondence::irtkPointCorrespondence(const irtkRegisteredPointSet *target,
                                                 const irtkRegisteredPointSet *source)
:
...
// -----------------------------------------------------------------------------
irtkPointCorrespondence::irtkPointCorrespondence(const irtkPointCorrespondence &other)
:
...
// -----------------------------------------------------------------------------
irtkPointCorrespondence::~irtkPointCorrespondence()
{
}
...
// -----------------------------------------------------------------------------
int irtkPointCorrespondence::GetPointDataIndexByCaseInsensitiveName(vtkPointData *pd, string name)
{
  transform(name.begin(), name.end(), name.begin(), ::tolower);
  for (int i = 0; i < pd->GetNumberOfArrays(); ++i) {
    const char *array_name = pd->GetArrayName(i);
    if (array_name == NULL) continue;
    string lower_name = array_name;
    transform(lower_name.begin(), lower_name.end(), lower_name.begin(), ::tolower);
    if (lower_name == name) return i;
  }
  return -1;
}

// -----------------------------------------------------------------------------
bool irtkPointCorrespondence::Set(const char *param, const char *value)
{
...
// -----------------------------------------------------------------------------
irtkParameterList irtkPointCorrespondence::Parameter() const
{
  irtkParameterList params = irtkObservable::Parameter();
...
static string TransformFeatureName(const char *name)
{
  ...
  transform(feature_name.begin(), feature_name.end(), feature_name.begin(), ::tolower);
...
// -----------------------------------------------------------------------------
bool irtkPointCorrespondence::AddFeature(const char *name, double weight, double slope, double intercept)
{
...
// -----------------------------------------------------------------------------
void irtkPointCorrespondence::RemoveFeature(const char *name)
{
  const string feature_name = TransformFeatureName(name);
...
// -----------------------------------------------------------------------------
void irtkPointCorrespondence::CompleteFeatureInfo(const irtkRegisteredPointSet *input, FeatureList &feature)
{
  vtkPointData * const inputPD = input->PointSet()->GetPointData();
  for (FeatureList::iterator i = feature.begin(); i != feature.end(); ++i) {
    if (i->_Index < -1) {
      if (i->_Name.empty()) {
        cerr << "irtkPointCorrespondence::CompleteFeatureInfo: Encountered feature without name" << endl;
        exit(1);
      }
      ...
      if (i->_Index < 0 && i->_Name != "eigenmodes" && i->_Name != "spectral normals") {
        cerr << "irtkPointCorrespondence::CompleteFeatureInfo: Missing point data " << i->_Name << endl;
        exit(1);
      }
    } else if (i->_Index >= inputPD->GetNumberOfArrays()) {
      cerr << "irtkPointCorrespondence::CompleteFeatureInfo: Feature index is out of bounds" << endl;
      exit(1);
    }
  }
}
...
// -----------------------------------------------------------------------------
void irtkPointCorrespondence::Initialize()
{
  ...
  if (!_FromTargetToSource && !_FromSourceToTarget) {
    cerr << "irtkPointCorrespondence::Initialize: At least one direction,"
  ...
  if (GetPointDimension(_Source, &_SourceFeatures) != _NumberOfFeatures) {
    cerr << "irtkPointCorrespondence::Initialize: Mismatching feature vector size" << endl;
    exit(1);
  }

  // Initialize this class
  irtkPointCorrespondence::Init();
}

// -----------------------------------------------------------------------------
void irtkPointCorrespondence::Reinitialize()
{
...
// -----------------------------------------------------------------------------
void irtkPointCorrespondence::Init()
{
  // Determine number of points (samples)
  _M = GetNumberOfPoints(_Target, _TargetSample);
  _N = GetNumberOfPoints(_Source, _SourceSample);
  if (_M == 0) {
    cerr << "irtkPointCorrespondence::Initialize: Target data set has no points!" << endl;
    exit(1);
  }
  if (_N == 0) {
    cerr << "irtkPointCorrespondence::Initialize: Source data set has no points!" << endl;
    exit(1);
  }
...
// -----------------------------------------------------------------------------
void irtkPointCorrespondence::Update()
{
...
// -----------------------------------------------------------------------------
bool irtkPointCorrespondence::Upgrade()
{
  return false;
}

// -----------------------------------------------------------------------------
int irtkPointCorrespondence::GetTargetIndex(int) const
{
  return -1;
}

// -----------------------------------------------------------------------------
int irtkPointCorrespondence::GetSourceIndex(int) const
{
  return -1;
}
...
// -----------------------------------------------------------------------------
void irtkPointCorrespondence::WriteDataSets(const char *prefix, const char *suffix, bool all) const
{
  if (all || debug >= 4) {
    const int sz = 1024;
    char      fname[sz];
...
// -----------------------------------------------------------------------------
void irtkPointCorrespondence::WriteSpectralPoints(const char *fname, vtkPointSet *d) const
{
