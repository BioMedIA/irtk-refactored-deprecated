...
void irtkFileNIFTIToImage::SetInput(const char *filename)
{
  ...
  if (strcmp(magic_number, "n+1") == 0) {
    ...
    if (strcmp(magic_number, "ni1") == 0) {
      // Parse img file name
      string ext = Extension(filename, EXT_LastWithGz);
      if (ext == ".hdr.gz") {
...
void irtkFileNIFTIToImage::ReadHeader()
{
  ...
  */
  ...
  } else if (hdr.nim->dim[0] == 5) {
    this->_attr._t  = hdr.nim->dim[5];
    this->_attr._dt = (hdr.nim->intent_code == NIFTI_INTENT_VECTOR ? 0 : 1);
  } else {
    this->_attr._t  = 1;
    this->_attr._dt = 1;
  }
  if (this->_attr._x < 1) this->_attr._x = 1;
  if (this->_attr._y < 1) this->_attr._y = 1;
  if (this->_attr._z < 1) this->_attr._z = 1;
  if (this->_attr._t < 1) this->_attr._t = 1;
  ...
  if      (hdr.nim->xyz_units == NIFTI_UNITS_METER)  xyz_scale = 1.0e3;
  else if (hdr.nim->xyz_units == NIFTI_UNITS_MICRON) xyz_scale = 1.0e-3;
  if (xyz_scale != 1.0) {
    this->_attr._dx      *= xyz_scale;
    this->_attr._dy      *= xyz_scale;
    this->_attr._dz      *= xyz_scale;
    this->_attr._xorigin *= xyz_scale;
    this->_attr._yorigin *= xyz_scale;
    this->_attr._zorigin *= xyz_scale;
  }

  // Convert temporal units to ms
  double t_scale = 1.0;
  if      (hdr.nim->time_units == NIFTI_UNITS_SEC)  t_scale = 1.0e3;
  else if (hdr.nim->time_units == NIFTI_UNITS_USEC) t_scale = 1.0e-3;
  if (t_scale != 1.0) {
    this->_attr._dt      *= t_scale;
    this->_attr._torigin *= t_scale;
  }
  ...
  if (hdr.nim->qform_code > 0 && hdr.nim->sform_code == NIFTI_XFORM_ALIGNED_ANAT) {
    for (int j = 0; j < 4; ++j) {
      for (int i = 0; i < 4; ++i) {
        this->_attr._smat(i, j) = hdr.nim->sto_xyz.m[i][j]; // sform = T * qform
        D_inv            (i, j) = hdr.nim->qto_ijk.m[i][j]; // inverse of qform
      }
    }
    this->_attr._smat *= D_inv; // isolate affine transformation T
  } else {
    this->_attr._smat.Ident();
  }
