...
void PrintHelp(const char *name)
{
  ...
  cout << "Usage: " << name << " <dofout> -dofnames <file> [options]" << endl;
  cout << endl;
  ...
  cout << endl;
  cout << "    T(x) = T_global(x) + T_local(x)" << endl;
  cout << endl;
  cout << "    Linear: T(x) = Ax,         T_global(x) =  A x, T_local(x) = 0" << endl;
  cout << "    FFD:    T(x) =  x + d(x),  T_global(x) = Id x, T_local(x) = d(x)" << endl;
  cout << "    MFFD:   T(x) = Ax + d(x),  T_global(x) =  A x, T_local(x) = d(x)" << endl;
  cout << "    Fluid:  T(x) =  x + d(Ax), T_global(x) =  A x, T_local(x) = d(Ax) - (Ax - x)" << endl;
  cout << endl;
  ...
  //               function irtkFluidFreeFormTransformation::LocalDisplacement and hence
  //               the virtual function irtkTransformation::LocalDisplacement can be used
  ...
  cout << "  instead using the barycentric fixed point iteration when the option -bi-invariant is given." << endl;
  ...
  cout << "  vectors by the inverse matrix of the global transformation [1]. Moreover, the (optional)" << endl;
  ...
  cout << endl;
  cout << "    d(x) = \\bar{A} inv(A) T_local(x)" << endl;
  cout << endl;
  ...
  cout << "  velocity fields, v(x) = log(d(x)), are averaged instead. If the input transformation" << endl;
  cout << "  itself is parameterized by stationary velocities, these velocities are used directly" << endl;
  cout << "  when possible to avoid the redundant log(exp) computation and its numerical error." << endl;
  ...
  cout << "  average displacement field, \\bar{d}(x)." << endl;
  cout << endl;
  ...
  cout << "  -dofdir <dir>              Directory used to make relative paths in -dofnames text file absolute. (default: cwd)" << endl;
  cout << "  -prefix <string>           Prefix for transformation name entries in -dofnames text file. (default: none)" << endl;
  cout << "  -suffix <string>           Suffix for transformation name entries in -dofnames text file. (default: none)" << endl;
  cout << "  -gaussian <mean> <sigma>   Use Gaussian kernel weights. Requires -dofnames to specify transformation values." << endl;
  cout << "                             By default, if -dofnames is used or <sigma> is 0, the values specified in the" << endl;
  ...
  cout << "                                      which is listed in the -dofnames list does not need to exist." << endl;
  cout << "                                      (default: read input transformation from file)" << endl;
  cout << endl;
  cout << "Average transformation options:" << endl;
  cout << "  -[no]rotation              Average rotation    or assume none to be present. (default: -norotation)" << endl;
  cout << "  -[no]translation           Average translation or assume none to be present. (default: -notranslation)" << endl;
  cout << "  -[no]scaling               Average scaling     or assume none to be present. (default: -scaling)" << endl;
  cout << "  -[no]shearing              Average shearing    or assume none to be present. (default: -shearing)" << endl;
  cout << "  -[no]deformation           Average deformation or assume none to be present. (default: -deformation)" << endl;
  cout << "  -[no]log                   Whether to average local transformations in log-space." << endl;
  cout << "                             (default: yes, unless the -dofs are averaged directly)" << endl;
  ...
  cout << endl;
  cout << "  -rigid                     Average rigid transformation parameters only." << endl;
  cout << "  -norigid                   Alias for -norotation, -notranslation." << endl;
  cout << "  -affine                    Average affine transformation parameters only." << endl;
  cout << "  -noaffine                  Alias for -norotation, -notranslation, -noscaling, -noshearing." << endl;
  cout << "  -all                       Average all transformation parameters." << endl;
  cout << endl;
...
int main(int argc, char **argv)
{
  ...
  irtkImageAttributes attr;  // common FFD lattice attributes
  ...
  // to define the image domain of the average displacement field
    ...
    for (size_t i = 0; i < dofin.size(); ++i) {
      ...
      irtkFreeFormTransformation *ffd = dynamic_cast<irtkFreeFormTransformation *>(p);
      if (ffd) {
        if (type.empty()) {
  ...
  if (avgdofs && deformation) {
    if (attr.NumberOfLatticePoints() > 0 && // input contains FFDs to be averaged
        type != "irtkBSplineFreeFormTransformationSV"   &&
        type != "irtkLinearFreeFormTransformation3D"    &&
        type != "irtkBSplineFreeFormTransformation3D") {
    ...
    if (logspace == 1) {
      if (type != "irtkBSplineFreeFormTransformationSV") {
        if (verbose) cout << endl;
        cerr << EXECNAME << ": Combining options -log and -dofs only possible for SV B-spline FFDs! Use only one of them." << endl;
        exit(1);
      }
    } else if (logspace == 0) {
      if (type != "irtkLinearFreeFormTransformation" && type != "irtkBSplineFreeFormTransformation3D") {
        if (verbose) cout << endl;
  ...
  irtkMatrix              *A = NULL;
  irtkGenericImage<double> d, avgD;
  ...
  for (size_t i = 0; i < dofin.size(); ++i) {
    ...
    irtkSimilarityTransformation    *similarity = NULL;
    irtkAffineTransformation        *affine     = NULL;
    irtkFreeFormTransformation3D    *ffd        = NULL;
    irtkMultiLevelTransformation    *mffd       = NULL;
    irtkFluidFreeFormTransformation *fluid      = NULL;
    if (!((fluid      = dynamic_cast<irtkFluidFreeFormTransformation *>(t.get())) ||
          (mffd       = dynamic_cast<irtkMultiLevelTransformation    *>(t.get())) ||
          (ffd        = dynamic_cast<irtkFreeFormTransformation3D    *>(t.get())) ||
          (affine     = dynamic_cast<irtkAffineTransformation        *>(t.get())) ||
          (similarity = dynamic_cast<irtkSimilarityTransformation    *>(t.get())) ||
          (rigid      = dynamic_cast<irtkRigidTransformation         *>(t.get())))) {
    ...
    // irtkRigidTransformation interface even if the actual transformation
    // is of type irtkSimilarityTransformation, irtkAffineTransformation,
    // or even a subclass of irtkMultiLevelTransformation.
    if (fluid) mffd = fluid;
    if (mffd) {
      ...
      if (mffd->NumberOfLevels() > 0) {
        ...
        ffd = dynamic_cast<irtkFreeFormTransformation3D *>(affd);
        if (!ffd) {
    ...
    if (deformation && ffd) {
      ...
      // Otherwise,...
        ...
        irtkMatrix i2w = attr.GetImageToWorldMatrix();
        for (int k = 0; k < attr._z; ++k) {
          for (int j = 0; j < attr._y; ++j) {
            for (int i = 0; i < attr._x; ++i) {
        ...
        if (logspace) {
          ...
          if (N < 5) {
            irtkGaussianBlurring<double> blur(max(attr._dx, max(attr._dy, attr._dz)));
            blur.SetInput (&d);
            blur.SetOutput(&d);
            blur.Run();
          }
  ...
  irtkAffineTransformation globalAvg;
  ...
  if (A) {
    ...
    if (verbose) {
      ...
      const irtkMatrix avgAInv = avgA.Inverse();
      irtkMatrix residual(4, 4);
      for (int i = 0; i < N; ++i) {
        if (w[i] == .0) continue;
  ...
  // Otherwise,...
    ...
    double x, y, z;
    const irtkMatrix &avgA = globalAvg.GetMatrix();
    for (int k = 0; k < attr._z; ++k) {
      for (int j = 0; j < attr._y; ++j) {
        for (int i = 0; i < attr._x; ++i) {
    ...
    if (avgdofs) {
      if (type == "irtkLinearFreeFormTransformation3D") {
        ffd = new irtkLinearFreeFormTransformation(avgD, true);
      } else if (type == "irtkBSplineFreeFormTransformation3D") {
        ffd = new irtkBSplineFreeFormTransformation(avgD, true);
      } else if (type == "irtkBSplineFreeFormTransformationSV") {
        ffd = new irtkBSplineFreeFormTransformationSV(avgD, true);
      }
    }
    if (!ffd) {
      if (bsplineffd) {
        ...
        ffd = new irtkBSplineFreeFormTransformation3D(avgD, true);
      } else {
        ffd = new irtkLinearFreeFormTransformation(avgD, true);
      }
    }
    ...
    irtkMultiLevelTransformation *mffd = NULL;
    if (mtype == "irtkMultiLevelStationaryVelocityTransformation") {
      mffd = new irtkMultiLevelStationaryVelocityTransformation(globalAvg);
    } else {
      mffd = new irtkMultiLevelFreeFormTransformation(globalAvg);
    }
