...
#ifndef _IRTKSPECTRALDECOMPOSITION_H
#define _IRTKSPECTRALDECOMPOSITION_H
...
typedef vector<pair<int, double> >      FeatureWeights;
...
class FindClosestPoints
{
  ...
  const irtkPointSet      &_P1;
  vtkAbstractPointLocator *_P2;
  vector<int>             &_Idx;

  FindClosestPoints(const irtkPointSet &, vtkAbstractPointLocator *, vector<int> &);
...
public:
  ...
  void operator ()(const blocked_range<int> &re) const
  {
    for (int i = re.begin(); i != re.end(); ++i) {
      const irtkPoint &p = _P1(i);
      _Idx[i] = static_cast<int>(_P2->FindClosestPoint(p._x, p._y, p._z));
    }
  }

  /// For each point in the first set, find closest point in the second set
  static vector<int> Run(const irtkPointSet &p1, const irtkPointSet &p2);
};
...
class FindClosestNPoints
{
  ...
  const irtkPointSet      &_P1;
  vtkAbstractPointLocator *_P2;
  vector<vector<int> >    &_Idx;
  int                      _Num;

  FindClosestNPoints(const irtkPointSet &, vtkAbstractPointLocator *, vector<vector<int> > &, int);
...
public:
  ...
  /// For each point in the first set, find closest point in the second set
  static vector<vector<int> > Run(const irtkPointSet &p1, const irtkPointSet &p2, int);
};
...
FeatureWeights EdgeWeights(vtkPolyData          *dataset,
                           ...
                           const vector<double> *weights = NULL);
...
void NormalizedLaplacian(SparseMatrix &L, const SparseMatrix &A,
                         const irtkVector *G = NULL, const irtkVector *D = NULL);
...
/// \oaram[in]  k Number of spectral components.
...
ToPointDataArray(const irtkMatrix &m, int r1 = 0,
...
int SetEigenmodes(vtkPolyData *d, const irtkMatrix &m, const char *name = "eigenmodes");

/// Set spectral components as point data of vtkPolyData instance
///
/// \returns Index of eigenmodes point data array.
int SetEigenmodes(vtkPolyData *d, const irtkMatrix &m,
                  int r1, int k = -1, const char *name = "eigenmodes");

/// Get spectral components from point data of vtkPolyData instance
irtkMatrix GetEigenmodes(vtkPolyData *d, const char *name = "eigenmodes");
