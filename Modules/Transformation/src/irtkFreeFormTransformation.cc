...
// -----------------------------------------------------------------------------
irtkFreeFormTransformation
::irtkFreeFormTransformation(CPInterpolator &func, CPInterpolator *func2D)
:
...
// -----------------------------------------------------------------------------
irtkFreeFormTransformation
::irtkFreeFormTransformation(const irtkFreeFormTransformation &ffd,
                             CPInterpolator &func, CPInterpolator *func2D)
:
  irtkTransformation(ffd),
...
irtkImageAttributes
irtkFreeFormTransformation
::DefaultAttributes(const irtkImageAttributes &attr,
                    double dx, double dy, double dz, double dt)
{
  ...
  const irtkImageAttributes grid = OrthogonalFieldOfView(attr);

  // Set lattice attributes to image attributes
  irtkImageAttributes lattice = grid;

  // Adjust lattice dimensions
  lattice._x = ((dx > .0 && grid._x > 1) ? static_cast<int>(round((grid._x - 1) * grid._dx / dx)) + 1 : 1);
  lattice._y = ((dy > .0 && grid._y > 1) ? static_cast<int>(round((grid._y - 1) * grid._dy / dy)) + 1 : 1);
  lattice._z = ((dz > .0 && grid._z > 1) ? static_cast<int>(round((grid._z - 1) * grid._dz / dz)) + 1 : 1);
  lattice._t = ((dt > .0 && grid._t > 1) ? static_cast<int>(round((grid._t - 1) * grid._dt / dt)) + 1 : 1);

  // Update lattice spacing
  if (version < irtkVersion(3, 2)) {
    lattice._dx = ((lattice._x > 1) ? ((grid._x - 1) * grid._dx) / (lattice._x - 1) : .0);
    lattice._dy = ((lattice._y > 1) ? ((grid._y - 1) * grid._dy) / (lattice._y - 1) : .0);
    lattice._dz = ((lattice._z > 1) ? ((grid._z - 1) * grid._dz) / (lattice._z - 1) : .0);
    lattice._dt = ((lattice._t > 1) ? ((grid._t - 1) * grid._dt) / (lattice._t - 1) : .0);
  } else {
                    ...
                    const double *zaxis)
{
  ...
  // Lattice dimensions
  attr._x = ((dx > .0) ? round((x2 - x1) / dx) + 1 : 1);
  attr._y = ((dy > .0) ? round((y2 - y1) / dy) + 1 : 1);
  attr._z = ((dz > .0) ? round((z2 - z1) / dz) + 1 : 1);
  attr._t = ((dt > .0) ? round((t2 - t1) / dt) + 1 : 1);

  // Lattice spacing
  if (version < irtkVersion(3, 2)) {
    attr._dx = ((attr._x > 1) ? ((x2 - x1) / (attr._x - 1)) : .0);
    attr._dy = ((attr._y > 1) ? ((y2 - y1) / (attr._y - 1)) : .0);
    attr._dz = ((attr._z > 1) ? ((z2 - z1) / (attr._z - 1)) : .0);
    attr._dt = ((attr._t > 1) ? ((t2 - t1) / (attr._t - 1)) : .0);
  } else {
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::InitializeInterpolator()
{
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::InitializeCPs(const irtkImageAttributes &attr, bool dofs)
{
  if (attr._x > 1000 || attr._y > 1000 || attr._z > 1000) {
    ...
    cerr << "irtkFreeFormTransformation::InitializeCPs: Number of control points may not exceed 1000 in each dimension!" << endl;
    exit(1);
  }
  Deallocate(_CPStatus, _Status);
  ...
  if (ncps > 0) {
    ...
    if (dofs) {
      ...
      if (sizeof(CPValue) != 3 * sizeof(DOFValue)) {
        cerr << "irtkFreeFormTransformation::InitializeCPs: Assertion \"sizeof(CPValue) == 3 * sizeof(DOFValue)\" failed!" << endl;
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::InitializeCPs(const irtkFreeFormTransformation &ffd, bool dofs)
{
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::InitializeStatus()
{
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::Initialize(const irtkImageAttributes &attr)
{
  InitializeCPs(attr);
  this->Changed(true);
}
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::Initialize(const CPImage &image, bool displacement)
{
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation
::Initialize(const irtkGenericImage<double> &image, bool displacement)
{
  if (image.T() < 2 || image.T() > 3) {
    cerr << "irtkFreeFormTransformation::Initialize:"
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::ExtrapolationMode(irtkExtrapolationMode mode)
{
...
// -----------------------------------------------------------------------------
bool irtkFreeFormTransformation::Set(const char *name, const char *value)
{
  // Extrapolation mode
  if (strcmp(name, "Transformation extrapolation") == 0) {
    irtkExtrapolationMode mode;
...
// -----------------------------------------------------------------------------
irtkParameterList irtkFreeFormTransformation::Parameter() const
{
...
// -----------------------------------------------------------------------------
irtkImageAttributes irtkFreeFormTransformation
::ApproximationDomain(const irtkImageAttributes &domain, const irtkTransformation *)
{
  return domain;
}

// -----------------------------------------------------------------------------
double irtkFreeFormTransformation
::EvaluateRMSError(const irtkTransformation *dof) const
{
  return EvaluateRMSError(_attr, dof);
}

// -----------------------------------------------------------------------------
double irtkFreeFormTransformation::Approximate(const irtkTransformation *t, int niter, double max_error)
{
  return this->Approximate(_attr, t, niter, max_error);
}

// -----------------------------------------------------------------------------
double irtkFreeFormTransformation::Approximate(const irtkImageAttributes &domain,
...
// -----------------------------------------------------------------------------
double irtkFreeFormTransformation::ApproximateAsNew(const irtkTransformation *t, int niter, double max_error)
{
...
// -----------------------------------------------------------------------------
double irtkFreeFormTransformation
::ApproximateAsNew(const irtkImageAttributes &domain, double *dx, double *dy, double *dz, int niter, double max_error)
{
  ...
  return irtkTransformation::ApproximateAsNew(domain, dx, dy, dz, niter, max_error);
}
...
// -----------------------------------------------------------------------------
bool irtkFreeFormTransformation::CropPadPassiveCPs(int margin, bool reset)
{
  return this->CropPadPassiveCPs(margin, margin, margin, margin, reset);
}

// -----------------------------------------------------------------------------
bool irtkFreeFormTransformation::CropPadPassiveCPs(int mx, int my, int mz, int mt, bool reset)
{
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::PutBoundingBox(double x1, double y1, double z1,
                                                double x2, double y2, double z2)
{
  const irtkImageAttributes &attr = Attributes();
  double a, b, c;
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::PutBoundingBox(const irtkPoint &p1, const irtkPoint &p2)
{
  PutBoundingBox(p1._x, p1._y, p1._z, p2._x, p2._y, p2._z);
}

// ---------------------------------------------------------------------------
void irtkFreeFormTransformation::PutBoundingBox(double t1, double t2)
{
...
// ---------------------------------------------------------------------------
void irtkFreeFormTransformation::PutBoundingBox(double x1, double y1, double z1, double t1,
                                                double x2, double y2, double z2, double t2)
{
...
// -----------------------------------------------------------------------------
/// (Multi-threaded) body of irtkFreeFormTransformation::ParametricGradient
///
/// This default implementation uses the irtkFreeFormTransformation::JacobianDOFs
...
/// are provided by the irtkFreeFormTransformation3D and
/// irtkFreeFormTransformation4D subclasses, which therefore override this
/// irtkFreeFormTransformation::ParametricGradient implementation. If a subclass
...
///
/// \code
/// void irtkBSplineFreeFormTransformationTD::ParametricGradient(...)
/// {
///   irtkFreeFormTransformation::ParametricGradient(...);
/// }
/// \endcode
class irtkFreeFormTransformationParametricGradientBody
{
public:
...
public:
  ...
  // ---------------------------------------------------------------------------
  /// Default constructor
  irtkFreeFormTransformationParametricGradientBody()
  :
    _FFD        (NULL),
    _Input      (NULL),
    _WorldCoords(NULL),
  ...
  // ---------------------------------------------------------------------------
  /// Split constructor
  irtkFreeFormTransformationParametricGradientBody(const irtkFreeFormTransformationParametricGradientBody &other, split)
  :
    _FFD        (other._FFD),
    _Input      (other._Input),
  ...
  void join(irtkFreeFormTransformationParametricGradientBody &rhs)
  {
    const int ndofs = _FFD->NumberOfDOFs();
  ...
  void operator ()(const blocked_range3d<int> &re)
  {
    ...
    irtkTransformationJacobian                      jac;
    irtkTransformationJacobian::ConstColumnIterator it;
  ...
  void operator ()()
  {
    ...
    if (_WorldCoords && (_WorldCoords->X() != _X ||
                         ...
                         _WorldCoords->Z() != _Z ||
                         _WorldCoords->T() !=  3)) {
      cerr << "irtkFreeFormTransformation::ParametricGradient: Invalid world coordinates map" << endl;
      exit(1);
    }
...
// -----------------------------------------------------------------------------
class irtkFreeFormTransformationPointWiseParametricGradientBody
{
public:
  ...
  // ---------------------------------------------------------------------------
  /// Default constructor
  irtkFreeFormTransformationPointWiseParametricGradientBody()
  :
    _FFD     (NULL),
    _PointSet(NULL),
    _Input   (NULL),
  ...
  // ---------------------------------------------------------------------------
  /// Split constructor
  irtkFreeFormTransformationPointWiseParametricGradientBody(const irtkFreeFormTransformationPointWiseParametricGradientBody &other, split)
  :
    _FFD     (other._FFD),
  ...
  // ---------------------------------------------------------------------------
  /// Join results of right-hand body with this body
  void join(irtkFreeFormTransformationPointWiseParametricGradientBody &rhs)
  {
    const int ndofs = _FFD->NumberOfDOFs();
    for (int dof = 0; dof < ndofs; ++dof) _Output[dof] += rhs._Output[dof];
    delete[] rhs._Output;
  ...
  void operator ()(const blocked_range<int> &re)
  {
    ...
    for (int i = re.begin(); i != re.end(); ++i) {
      const irtkVector3D<double> &g = _Input[i];
...
{
  IRTK_START_TIMING();
  irtkFreeFormTransformationParametricGradientBody body;
  body._FFD         = this;
  body._Input       = in;
  body._Output      = out;
  body._Weight      = w;
  ...
  body._t0          = t0;
  body();
  IRTK_DEBUG_TIMING(2, "parametric gradient computation (FFD)");
}
...
{
  IRTK_START_TIMING();
  irtkFreeFormTransformationPointWiseParametricGradientBody body;
  body._FFD      = this;
  body._PointSet = &pos;
  body._Input    = in;
  body._Output   = out;
  ...
  body._t0       = t0;
  body();
  IRTK_DEBUG_TIMING(2, "point-wise parametric gradient computation (FFD)");
}
...
// -----------------------------------------------------------------------------
double irtkFreeFormTransformation::BendingEnergy(double x, double y, double z, double t, double t0, bool wrt_world) const
{
  if (!wrt_world) {
    cerr << "irtkFreeFormTransformation::BendingEnergy: Always uses derivatives w.r.t. world coordinates" << endl;
    exit(1);
  }
...
// -----------------------------------------------------------------------------
double irtkFreeFormTransformation::BendingEnergy(bool incl_passive, bool wrt_world) const
{
  int    nactive = 0;
  double bending = .0;
  double x, y, z, t;
...
// -----------------------------------------------------------------------------
double irtkFreeFormTransformation::BendingEnergy(const irtkImageAttributes &attr, double t0, bool wrt_world) const
{
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::BendingEnergyGradient(double *, double, bool, bool) const
{
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation::Print(irtkIndent indent) const
{
...
// -----------------------------------------------------------------------------
irtkCofstream &irtkFreeFormTransformation::WriteCPs(irtkCofstream &to) const
{
...
// -----------------------------------------------------------------------------
double irtkFreeFormTransformation::Bending(double x, double y, double z) const
{
  cerr << "irtkFreeFormTransformation::Bending: Deprecated function not implemented by subclass" << endl;
  exit(1);
  return .0;
}
