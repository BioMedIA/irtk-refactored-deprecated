...
class irtkRegistrationEnergyParser
{
  irtkGenericRegistrationFilter *_Filter;

  typedef irtkGenericRegistrationFilter::TransformationInfo     TransformationInfo;
  typedef irtkGenericRegistrationFilter::ImageSimilarityInfo    ImageSimilarityInfo;
  typedef irtkGenericRegistrationFilter::PointSetDistanceInfo   PointSetDistanceInfo;
  typedef irtkGenericRegistrationFilter::PointSetConstraintInfo PointSetConstraintInfo;
  typedef irtkGenericRegistrationFilter::ConstraintInfo         ConstraintInfo;
  ...
  vector<int> InputIndex(istream &in, Token &token, const string &name,
                         DataType type, int max_index)
  {
    ...
    switch (type) {
      ...
      case POLYDATA:
        ...
        break;
      default:
        cerr << "irtkRegistrationEnergyParser::InputIndex: Invalid data type identifier: " << type << endl;
        exit(1);
    }
...
protected:
  ...
  void ParseEnergyTerm(istream &in, Token &token, int nimages, int npsets)
  {
    ...
    irtkSimilarityMeasure         similarity = _Filter->_SimilarityMeasure;
    irtkPointSetDistanceMeasure   pdm        = _Filter->_PointSetDistanceMeasure;
    irtkPointSetConstraintMeasure pcm;
    irtkConstraintMeasure         constraint;

    term_is_[SIM] = ((name == "SIM") || FromString(name.c_str(), similarity));
    term_is_[PDM] = ((name == "PDM") || FromString(name.c_str(), pdm));
    term_is_[PCM] = (                   FromString(name.c_str(), pcm));
    ...
    if (term_is_[SIM]) {
      ...
      if (source_index.size() > 0) {
        ...
        for (size_t t = 0; t < target_index.size(); ++t) {
          ...
          info._RefPointSetIndex = -1;
          info._RefImageIndex    = -1;
          // {i} substituted by irtkGenericRegistrationFilter::AddPointSetConstraint
...
public:
  ...
  // ---------------------------------------------------------------------------
  /// Constructor
  irtkRegistrationEnergyParser(irtkGenericRegistrationFilter *filter)
  :
    _Filter(filter)
  {}
