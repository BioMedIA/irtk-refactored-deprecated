...
// -----------------------------------------------------------------------------
irtkCurrentsDistance::irtkCurrentsDistance(const char *name, double weight)
:
...
// -----------------------------------------------------------------------------
irtkCurrentsDistance::irtkCurrentsDistance(const irtkCurrentsDistance &other)
:
...
// -----------------------------------------------------------------------------
irtkCurrentsDistance &irtkCurrentsDistance::operator =(const irtkCurrentsDistance &other)
{
  irtkPointSetDistance::operator =(other);
  _TargetNormSquared = other._TargetNormSquared;
  if (other._TargetCurrent) {
    _TargetCurrent = vtkSmartPointer<vtkPolyData>::New();
    _TargetCurrent->DeepCopy(other._TargetCurrent);
...
// -----------------------------------------------------------------------------
irtkCurrentsDistance::~irtkCurrentsDistance()
{
}
...
// -----------------------------------------------------------------------------
vtkSmartPointer<vtkPolyData> irtkCurrentsDistance::SurfaceToCurrent(vtkPolyData *data)
{
  IRTK_START_TIMING();
  ...
  for (vtkIdType i = 0; i < num_faces; ++i) {
    ...
    if (npts != 3) {
      cerr << "irtkCurrentsDistance::SurfaceToCurrent: Surface cells must have three points each!" << endl;
      exit(1);
    }
  ...
  IRTK_DEBUG_TIMING(3, "conversion of surface mesh to current");
  return current;
}

// -----------------------------------------------------------------------------
vtkSmartPointer<vtkPolyData> irtkCurrentsDistance::ToCurrent(vtkPointSet *data)
{
  ...
  if (surface->GetPolys()->GetNumberOfCells() == 0) {
    cerr << "irtkCurrentsDistance::ToCurrent: Only surface meshes supported at the moment" << endl;
    exit(1);
  }
...
// -----------------------------------------------------------------------------
class irtkCurrentsDistanceDotProduct
{
private:
...
public:
  ...
  irtkCurrentsDistanceDotProduct(double sigma)
  :
  ...
  inline double Evaluate(vtkPolyData *a, vtkPolyData *b, vtkDataArray *value = NULL)
  {
    ...
    IRTK_DEBUG_TIMING(3, "evaluation of dot product of currents");
    return _Sum;
  }

/* private: */

  irtkCurrentsDistanceDotProduct(irtkCurrentsDistanceDotProduct &other, split)
  :
  ...
  void join(irtkCurrentsDistanceDotProduct &other)
  {
    _Sum += other._Sum;
  }
...
// -----------------------------------------------------------------------------
class irtkCurrentsDistanceGradient
{
private:
...
public:
  ...
  irtkCurrentsDistanceGradient(double sigma)
  :
                               ...
                               irtkVector3D<double> *g)
  {
    ...
    _LocatorB->Delete(), _LocatorB = NULL;
    IRTK_DEBUG_TIMING(3, "evaluation of gradient of currents distance");
  }

/* private: */

  irtkCurrentsDistanceGradient(irtkCurrentsDistanceGradient &other)
  :
  ...
  irtkCurrentsDistanceGradient(irtkCurrentsDistanceGradient &lhs, split)
  :
  ...
  void join(irtkCurrentsDistanceGradient &rhs)
  {
    for (int i = 0; i < _SurfaceA->GetNumberOfPoints(); ++i) {
  ...
  inline void Add(irtkVector3D<double> &v, double g[3])
  {
  ...
  void operator ()(const blocked_range<vtkIdType> &re)
  {
    ...
    for (vtkIdType i = re.begin(); i != re.end(); ++i) {
      ...
      i2 = ids->GetId(1);
      i3 = ids->GetId(2);
      irtkVector3D<double> &g1 = _Gradient[i1];
      irtkVector3D<double> &g2 = _Gradient[i2];
      irtkVector3D<double> &g3 = _Gradient[i3];
...
// -----------------------------------------------------------------------------
void irtkCurrentsDistance::Init()
{
  // Derive kernel sigma value, assuming points distributed in a spherical arrangement
  if (_Sigma == .0) {
    cerr << "irtkCurrentsDistance::Initialize: Kernel width may not be zero!" << endl;
    exit(1);
  }
  if (_Sigma < .0) {
    double ba[6], bb[6];
  ...
  _TargetNormSquared = .0;
  irtkCurrentsDistanceDotProduct dot_product(_Sigma);
...
// -----------------------------------------------------------------------------
void irtkCurrentsDistance::Initialize()
{
  // Initialize base class
  irtkPointSetDistance::Initialize();
  // Initialize this class
  irtkCurrentsDistance::Init();
}

// -----------------------------------------------------------------------------
void irtkCurrentsDistance::Reinitialize()
{
  // Reinitialize base class
  irtkPointSetDistance::Reinitialize();
...
// -----------------------------------------------------------------------------
bool irtkCurrentsDistance::Set(const char *param, const char *value)
{
  if (strcmp(param, "Currents kernel width") == 0) {
    return FromString(value, _Sigma) && _Sigma != .0;
  } else if (strcmp(param, "Symmetric currents distance") == 0) {
    return FromString(value, _Symmetric);
  } else if (!_Name.empty()) {
    if (_Name + " kernel width" == param) {
      return FromString(value, _Sigma) && _Sigma != .0;
    } else if (_Name + " symmetric" == param) {
      return FromString(value, _Symmetric);
    }
  }
  return irtkPointSetDistance::Set(param, value);
}

// -----------------------------------------------------------------------------
irtkParameterList irtkCurrentsDistance::Parameter() const
{
  irtkParameterList params = irtkPointSetDistance::Parameter();
  if (!_Name.empty()) {
    Insert(params, _Name + " kernel width", ToString(_Sigma));
    Insert(params, _Name + " symmetric",    ToString(_Symmetric));
  }
  return params;
}
...
// -----------------------------------------------------------------------------
void irtkCurrentsDistance::Update(bool)
{
  if (_Target->Transformation()) {
    _Target->Update();
...
// -----------------------------------------------------------------------------
double irtkCurrentsDistance::Evaluate()
{
  IRTK_START_TIMING();
  ...
  IRTK_DEBUG_TIMING(2, "evaluation of currents distance");
  return d / ((_TargetCurrent->GetNumberOfPoints() + _SourceCurrent->GetNumberOfPoints()) / 2);
}

// -----------------------------------------------------------------------------
void irtkCurrentsDistance::NonParametricGradient(const irtkRegisteredPointSet *target,
                                                 GradientType                 *gradient)
{
  ...
  irtkCurrentsDistanceGradient d(_Sigma);
...
// -----------------------------------------------------------------------------
void irtkCurrentsDistance::WriteDataSets(const char *p, const char *suffix, bool all) const
{
  const int   sz = 1024;
  char        fname[sz];
