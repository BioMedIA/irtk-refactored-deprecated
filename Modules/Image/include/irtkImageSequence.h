...
#ifndef _IRTKIMAGESEQUENCE_H
#define _IRTKIMAGESEQUENCE_H
 ...
 */
template <class TImage = irtkBaseImage>
class irtkImageChannel : public irtkObject
{
  irtkObjectMacro(irtkImageChannel);
...
public:
  ...
  irtkImageChannel(ImageType *image = NULL, bool manage = false, bool copy = false);
  ...
  ///            This is required for using irtkImageChannel as type for an STL
  ///            container which creates copies of the elements.
  irtkImageChannel(const irtkImageChannel &);
  ...
  ///            container which creates copies of the elements.
  ///
  /// \note Implemented only for TImage template argument irtkBaseImage.
  template <class TOtherImage>
  irtkImageChannel(const irtkImageChannel<TOtherImage> &);
  ...
  ///            This is required for using irtkImageChannel as type for an STL
  ///            container which creates copies of the elements.
  irtkImageChannel &operator =(const irtkImageChannel &);
  ...
  /// \note Implemented only for TImage template argument irtkBaseImage.
  template <class TOtherImage>
  irtkImageChannel &operator =(const irtkImageChannel<TOtherImage> &);

  /// Destructor
  virtual ~irtkImageChannel();
...
protected:
  ...
  template <class T> friend class irtkImageChannel;
};
 ...
 */
template <class TChannel = irtkImageChannel<irtkBaseImage> >
class irtkImageFrame : public irtkObject
{
  irtkObjectMacro(irtkImageFrame);
...
public:
  ...
  /// Copy constructor
  irtkImageFrame(const irtkImageFrame &);

  /// Assignment operator
  irtkImageFrame &operator =(const irtkImageFrame &);

  /// Destructor
  virtual ~irtkImageFrame();
  ...
  irtkImageAttributes Attributes() const;
...
private:
  ...
  vector<ChannelType> _Channel; ///< Channels of image frame
 ...
 */
template <class TFrame = irtkImageFrame<> >
class irtkImageSequence : public irtkObject
{
  irtkObjectMacro(irtkImageSequence);
...
public:
  ...
  /// Copy constructor
  irtkImageSequence(const irtkImageSequence &);

  /// Assignment operator
  irtkImageSequence &operator =(const irtkImageSequence &);
...
private:
  ...
  vector<FrameType> _Frame; ///< Frames of image frame

};

// Inline definitions
#include <irtkImageSequence.hh>
