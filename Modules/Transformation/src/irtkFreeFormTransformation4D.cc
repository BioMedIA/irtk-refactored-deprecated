...
// -----------------------------------------------------------------------------
irtkFreeFormTransformation4D::irtkFreeFormTransformation4D(CPInterpolator &func)
:
  irtkFreeFormTransformation(func)
{
}

// -----------------------------------------------------------------------------
irtkFreeFormTransformation4D
::irtkFreeFormTransformation4D(const irtkFreeFormTransformation4D &ffd,
                               CPInterpolator                     &func)
:
  irtkFreeFormTransformation(ffd, func)
{
}

// -----------------------------------------------------------------------------
irtkFreeFormTransformation4D::~irtkFreeFormTransformation4D()
{
}

// -----------------------------------------------------------------------------
void irtkFreeFormTransformation4D::Initialize(const irtkImageAttributes &attr)
{
  if (attr._dt == 0) {
...
// -----------------------------------------------------------------------------
/// (Multi-threaded) body of irtkFreeFormTransformation4D::ParametricGradient
///
...
class irtkFreeFormTransformation4DParametricGradientBody
{
public:
  const irtkFreeFormTransformation4D *_FFD;
  ...
  void operator ()(const blocked_range3d<int> &re) const
  {
    ...
    irtkMatrix    jac;                  // Jacobian of transformation w.r.t control point
  ...
  void operator ()()
  {
    ...
    if (_Image2World && (_Image2World->X() != _X ||
                         ...
                         _Image2World->Z() != _Z ||
                         _Image2World->T() !=  3)) {
      cerr << "irtkFreeFormTransformation4D::ParametricGradient: Invalid voxel coordinates map" << endl;
      exit(1);
    }
    if (_WorldCoords && (_WorldCoords->X() != _X ||
                         ...
                         _WorldCoords->Z() != _Z ||
                         _WorldCoords->T() !=  3)) {
      cerr << "irtkFreeFormTransformation4D::ParametricGradient: Invalid world coordinates map" << endl;
      exit(1);
    }
    ...
    for (_cl = 0; _cl < _FFD->T(); _cl++) {
      ...
      parallel_for(cps, irtkFreeFormTransformation4DParametricGradientBody(*this));
    }
  }

}; // irtkFreeFormTransformation4DParametricGradientBody

// -----------------------------------------------------------------------------
void irtkFreeFormTransformation4D
...
{
  IRTK_START_TIMING();
  irtkFreeFormTransformation4DParametricGradientBody body;
  body._FFD         = this;
  body._Input       = in;
  body._Output      = out;
  body._Weight      = w;
  ...
  body._t0          = t0;
  body();
  IRTK_DEBUG_TIMING(2, "parametric gradient computation (4D FFD)");
}
...
// -----------------------------------------------------------------------------
irtkCifstream &irtkFreeFormTransformation4D::ReadDOFs(irtkCifstream &from, irtkTransformationType format)
{
      ...
      for (int j = 0; j < attr._y; ++j) {
        for (int k = 0; k < attr._z; ++k) {
          for (int l = 0; l < attr._t; ++l) {
            _CPImage(i, j, k, l) = CPValue(param[dof    ],
                                           param[dof + 1],
                                           param[dof + 2]);
            dof += 3;
          }
...
// -----------------------------------------------------------------------------
irtkCofstream &irtkFreeFormTransformation4D::WriteDOFs(irtkCofstream &to) const
{
