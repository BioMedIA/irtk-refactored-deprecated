...
:
  irtkFreeFormTransformation4D(_FFD)
{
}
...
// -----------------------------------------------------------------------------
irtkBSplineFreeFormTransformation4D
::irtkBSplineFreeFormTransformation4D(const irtkImageAttributes &attr,
                                      double dx, double dy, double dz, double dt)
:
  irtkFreeFormTransformation4D(_FFD)
{
...
// -----------------------------------------------------------------------------
irtkBSplineFreeFormTransformation4D
::irtkBSplineFreeFormTransformation4D(const irtkBaseImage &target,
                                      double dx, double dy, double dz, double dt)
:
  irtkFreeFormTransformation4D(_FFD)
{
...
// -----------------------------------------------------------------------------
irtkBSplineFreeFormTransformation4D
::irtkBSplineFreeFormTransformation4D(const irtkBSplineFreeFormTransformation4D &ffd)
:
  irtkFreeFormTransformation4D(ffd, _FFD)
{
...
// -----------------------------------------------------------------------------
void irtkBSplineFreeFormTransformation4D
...
// -----------------------------------------------------------------------------
void irtkBSplineFreeFormTransformation4D::Interpolate(const double *dx, const double *dy, const double *dz)
{
...
// -----------------------------------------------------------------------------
int irtkBSplineFreeFormTransformation4D::GetXAfterSubdivision() const
{
  return (_x == 1 ? _x : 2 * _x - 1);
}

// -----------------------------------------------------------------------------
int irtkBSplineFreeFormTransformation4D::GetYAfterSubdivision() const
{
  return (_y == 1 ? _y : 2 * _y - 1);
}

// -----------------------------------------------------------------------------
int irtkBSplineFreeFormTransformation4D::GetZAfterSubdivision() const
{
  return (_z == 1 ? _z : 2 * _z - 1);
}

// -----------------------------------------------------------------------------
int irtkBSplineFreeFormTransformation4D::GetTAfterSubdivision() const
{
  return (_t == 1 ? _t : 2 * _t - 1);
}

// -----------------------------------------------------------------------------
double irtkBSplineFreeFormTransformation4D::GetXSpacingAfterSubdivision() const
{
  return (_x == 1 ? _dx : 0.5 * _dx);
}

// -----------------------------------------------------------------------------
double irtkBSplineFreeFormTransformation4D::GetYSpacingAfterSubdivision() const
{
  return (_y == 1 ? _dy : 0.5 * _dy);
}

// -----------------------------------------------------------------------------
double irtkBSplineFreeFormTransformation4D::GetZSpacingAfterSubdivision() const
{
  return (_z == 1 ? _dz : 0.5 * _dz);
}

// -----------------------------------------------------------------------------
double irtkBSplineFreeFormTransformation4D::GetTSpacingAfterSubdivision() const
{
  return (_t == 1 ? _dt : 0.5 * _dt);
}

// -----------------------------------------------------------------------------
void irtkBSplineFreeFormTransformation4D::Subdivide(bool subdivide_x, bool subdivide_y, bool subdivide_z, bool subdivide_t)
{
...
void EvaluateJacobian(const CPImage *coeff, irtkMatrix &jac, int i, int j, int k, int l)
{
  typedef irtkBSplineFreeFormTransformation4D::Kernel Kernel;
...
// -----------------------------------------------------------------------------
void irtkBSplineFreeFormTransformation4D
::EvaluateJacobian(irtkMatrix &jac, int i, int j, int k, int l) const
{
  if (_FFD.IsInside(i, j, k, l)) ::EvaluateJacobian(&_CPImage, jac, i, j, k, l);
  else                           ::EvaluateJacobian( _CPValue, jac, i, j, k, l);
}
...
//       (the full Jacobian is a 3x4 matrix and we return a 3x3 one)
template <class CPImage>
void EvaluateJacobian(const CPImage *coeff, irtkMatrix &jac, double x, double y, double z, double t)
{
  typedef irtkBSplineFreeFormTransformation4D::Kernel Kernel;
...
{
  if (_FFD.IsInside(x, y, z, t)) ::EvaluateJacobian(&_CPImage, jac, x, y, z, t);
  else                           ::EvaluateJacobian( _CPValue, jac, x, y, z, t);
}

// -----------------------------------------------------------------------------
template <class CPImage>
void EvaluateHessian(const CPImage *coeff, irtkMatrix hessian[3], int i, int j, int k, int l)
{
  typedef irtkBSplineFreeFormTransformation4D::Kernel Kernel;
...
// -----------------------------------------------------------------------------
void irtkBSplineFreeFormTransformation4D
::EvaluateHessian(irtkMatrix hessian[3], int i, int j, int k, int l) const
{
  if (_FFD.IsInside(i, j, k, l)) ::EvaluateHessian(&_CPImage, hessian, i, j, k, l);
  else                           ::EvaluateHessian( _CPValue, hessian, i, j, k, l);
}

// -----------------------------------------------------------------------------
template <class CPImage>
void EvaluateHessian(const CPImage *coeff, irtkMatrix hessian[3], double x, double y, double z, double t)
{
  typedef irtkBSplineFreeFormTransformation4D::Kernel Kernel;
...
// -----------------------------------------------------------------------------
void irtkBSplineFreeFormTransformation4D
::EvaluateHessian(irtkMatrix hessian[3], double x, double y, double z, double t) const
{
  if (_FFD.IsInside(x, y, z, t)) ::EvaluateHessian(&_CPImage, hessian, x, y, z, t);
  else                           ::EvaluateHessian( _CPValue, hessian, x, y, z, t);
}
...
{
  if (_FFD.IsInside(i, j, k, l)) ::EvaluateLaplacian(&_CPImage, laplacian, i, j, k, l);
  else                           ::EvaluateLaplacian( _CPValue, laplacian, i, j, k, l);
}
...
{
  if (_FFD.IsInside(i, j, k, t)) ::EvaluateLaplacian(&_CPImage, laplacian, i, j, k, t);
  else                           ::EvaluateLaplacian( _CPValue, laplacian, i, j, k, t);
}
...
{
  if (_FFD.IsInside(x, y, z, t)) ::EvaluateLaplacian(&_CPImage, x, y, z, t);
  else                           ::EvaluateLaplacian( _CPValue, x, y, z, t);
...
// -----------------------------------------------------------------------------
class irtkBSplineFreeFormTransformation4DParametricGradientBody
{
  typedef irtkBSplineFreeFormTransformation4D::Kernel Kernel;

public:
  const irtkBSplineFreeFormTransformation4D *_FFD;
  ...
  void operator ()(const blocked_range3d<int> &re) const
  {
    ...
    irtkFreeFormTransformation4D::CPStatus status;
  ...
  void operator ()()
  {
    ...
    if (_Image2World && (_Image2World->X() != _X || _Image2World->Y() != _Y ||
      ...
      cerr << "irtkBSplineFreeFormTransformation4D::ParametricGradient: Invalid voxel coordinates map" << endl;
      exit(1);
    }
    if (_WorldCoords && (_WorldCoords->X() != _X || _WorldCoords->Y() != _Y ||
                         _WorldCoords->Z() !=  Z || _WorldCoords->T() !=  3)) {
      cerr << "irtkBSplineFreeFormTransformation4D::ParametricGradient: Invalid world coordinates map" << endl;
      exit(1);
    }
    ...
    irtkExtrapolationMode m = _FFD->ExtrapolationMode();
    ...
    for (_cl = 0; _cl < _FFD->T(); _cl++) {
      ...
      parallel_for(cps, irtkBSplineFreeFormTransformation4DParametricGradientBody(*this));
    }
  }

}; // irtkBSplineFreeFormTransformation4DParametricGradientBody
...
{
  IRTK_START_TIMING();
  irtkBSplineFreeFormTransformation4DParametricGradientBody body;
  body._FFD         = this;
  body._Input       = in;
  body._Output      = out;
  body._Weight      = w;
  ...
  body();
  IRTK_DEBUG_TIMING(2, "parametric gradient computation (4D B-spline FFD)");
}
...
// -----------------------------------------------------------------------------
int irtkBSplineFreeFormTransformation4D::KernelSize() const
{
  return 4;
}

// -----------------------------------------------------------------------------
double irtkBSplineFreeFormTransformation4D::BendingEnergy(double x, double y, double z, double t, double, bool wrt_world) const
{
...
// -----------------------------------------------------------------------------
double irtkBSplineFreeFormTransformation4D::BendingEnergy(bool incl_passive, bool wrt_world) const
{
  int        nactive = 0;
  double     bending = .0;
  irtkMatrix hessian[3];
...
// -----------------------------------------------------------------------------
double irtkBSplineFreeFormTransformation4D::BendingEnergy(const irtkImageAttributes &attr, double, bool wrt_world) const
{
...
namespace irtkBSplineFreeFormTransformation4DUtils {
...
{
  typedef irtkBSplineFreeFormTransformation4D::CPExtrapolator Extrapolator;
  typedef irtkBSplineFreeFormTransformation4D::Vector         Vector;
  typedef irtkBSplineFreeFormTransformation4D::Kernel         Kernel;

  const irtkBSplineFreeFormTransformation4D *_Transformation; ///< B-spline free-form deformation
  ...
  /// Constructor
  Evaluate2ndOrderBSplineFFDDerivatives(const irtkBSplineFreeFormTransformation4D *ffd,
...
using namespace irtkBSplineFreeFormTransformation4DUtils;


// -----------------------------------------------------------------------------
void irtkBSplineFreeFormTransformation4D::BendingEnergyGradient(double *gradient, double weight, bool incl_passive, bool wrt_world) const
{
  Vector sum;
  int    m, n;
...
// -----------------------------------------------------------------------------
void irtkBSplineFreeFormTransformation4D::Print(irtkIndent indent) const
{
  cout << indent << "4D B-spline FFD:" << endl;
  irtkFreeFormTransformation4D::Print(indent + 1);
}

// -----------------------------------------------------------------------------
bool irtkBSplineFreeFormTransformation4D::CanRead(irtkTransformationType format) const
{
  switch (format) {
