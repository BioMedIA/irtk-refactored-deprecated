...
int main(int argc, char *argv[])
{
  ...
  const char *x_name = "X";
  const char *f_name = "F";
  vector<pair<const char *, const char *> > pd_names, cd_names;
  ...
  for (ALL_OPTIONS) {
    if (OPTION("-points")) {
      ...
      pd_names.push_back(make_pair(name, var));
    }
    else if (OPTION("-celldata")) {
      const char *name = ARGUMENT;
      const char *var  = (HAS_ARGUMENT ? ARGUMENT : name);
      cd_names.push_back(make_pair(name, var));
    }
  ...
  // Read input dataset
  if (verbose > 1) cout << "Reading dataset from " << vtk_name << endl;
  vtkSmartPointer<vtkPolyData> polydata = ReadPolyData(vtk_name);
  if (polydata == NULL) {
    cerr << "Error: Failed to read input dataset!" << endl;
    exit(1);
  }
  ...
  if (x_name) {
    double x[3];
    irtkMatrix X(polydata->GetNumberOfPoints(), 3);
    for (vtkIdType i = 0; i < polydata->GetNumberOfPoints(); ++i) {
      polydata->GetPoint(i, x);
      X(i, 0) = x[0], X(i, 1) = x[1], X(i, 2) = x[2];
    }
    mxArray *var = X.MxArray();
    if (matPutVariable(fp, x_name, var) != 0) {
      cerr << "Failed to write points to MAT file!" << endl;
      mxDestroyArray(var);
      matClose(fp);
      exit(1);
    }
  ...
  if (f_name) {
    ...
    if (polydata->GetPolys()->GetMaxCellSize() != 3) {
      cerr << "Error: Input dataset must be triangulated surface mesh!" << endl;
      exit(1);
    }
    irtkMatrix F(polydata->GetNumberOfCells(),  3);
    vtkIdType i = 0;
    ...
    mxArray *var = F.MxArray();
    if (matPutVariable(fp, x_name, var) != 0) {
      cerr << "Failed to write faces to MAT file!" << endl;
      mxDestroyArray(var);
      matClose(fp);
      exit(1);
    }
  ...
  for (size_t i = 0; i < pd_names.size(); ++i) {
    ...
    if (!array) {
      cerr << "Input polydata has no point data array named " << pd_names[i].first << endl;
      matClose(fp);
      exit(1);
    }
    irtkMatrix S(array->GetNumberOfTuples(), array->GetNumberOfComponents());
    for (vtkIdType i = 0; i < array->GetNumberOfTuples(); ++i) {
      for (int j = 0; j < array->GetNumberOfComponents(); ++j) {
        S(i, j) = array->GetComponent(i, j);
      }
    }
    mxArray *var = S.MxArray();
    if (matPutVariable(fp, var_name.c_str(), var) != 0) {
      cerr << "Failed to write point data array " << pd_names[i].first << " to MAT file!" << endl;
      mxDestroyArray(var);
      matClose(fp);
      exit(1);
    }
  ...
  for (size_t i = 0; i < cd_names.size(); ++i) {
    ...
    if (!array) {
      cerr << "Input polydata has no cell data array named " << cd_names[i].first << endl;
      matClose(fp);
      exit(1);
    }
    irtkMatrix S(array->GetNumberOfTuples(), array->GetNumberOfComponents());
    ...
    mxArray *var = S.MxArray();
    if (matPutVariable(fp, var_name.c_str(), var) != 0) {
      cerr << "Failed to write cell data array " << cd_names[i].first << " to MAT file!" << endl;
      mxDestroyArray(var);
      matClose(fp);
      exit(1);
    }
