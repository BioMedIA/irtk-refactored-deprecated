...
#include <irtkHarmonicVolumeParameterizer.h> // used to obtain initial map
...
namespace irtkAsConformalAsPossibleVolumeParameterizerUtils {
...
public:
  ...
  typedef Eigen::JacobiSVD<Matrix3x3, Eigen::NoQRPreconditioner> SVDSolver;

  vtkPointSet           *_PointSet;
  vtkDataArray          *_Coords;
  vector<irtkMatrix3x3> *_Orientation;

  // ---------------------------------------------------------------------------
  /// Add tensor product of 3D vectors to given 3x3 matrix
  static inline void AddTensorProduct(const double a[3], const double b[3], Matrix3x3 &m)
  {
    m(0, 0) += a[0] * b[0];
    m(0, 1) += a[0] * b[1];
    m(0, 2) += a[0] * b[2];
  ...
  static inline double Det(const Matrix3x3& m)
  {
  ...
  void operator()(const blocked_range<vtkIdType> &re) const
  {
    ...
    for (vtkIdType cellId = re.begin(); cellId != re.end(); ++cellId) {
      ...
      (*_Orientation)[cellId] = irtkMatrix3x3(rot(0, 0), rot(0, 1), rot(0, 2),
...
// namespace irtkAsConformalAsPossibleVolumeParameterizerUtils
using namespace irtkAsConformalAsPossibleVolumeParameterizerUtils;
...
// -----------------------------------------------------------------------------
void irtkAsConformalAsPossibleVolumeParameterizer
::Copy(const irtkAsConformalAsPossibleVolumeParameterizer &other)
{
...
// -----------------------------------------------------------------------------
irtkAsConformalAsPossibleVolumeParameterizer::irtkAsConformalAsPossibleVolumeParameterizer()
:
  _UniformWeight(.7)
{
}
...
::irtkAsConformalAsPossibleVolumeParameterizer(const irtkAsConformalAsPossibleVolumeParameterizer &other)
:
  irtkLinearVolumeParameterizer(other)
{
  Copy(other);
}
...
// -----------------------------------------------------------------------------
irtkAsConformalAsPossibleVolumeParameterizer &irtkAsConformalAsPossibleVolumeParameterizer
::operator =(const irtkAsConformalAsPossibleVolumeParameterizer &other)
{
  irtkLinearVolumeParameterizer::operator =(other);
  Copy(other);
  return *this;
}
...
// -----------------------------------------------------------------------------
void irtkAsConformalAsPossibleVolumeParameterizer::Initialize()
{
  const int d = 3; // dimension of output domain

  // Initialize base class
  irtkLinearVolumeParameterizer::Initialize();
  ...
  if (no_initial_map) {
    irtkHarmonicVolumeParameterizer harmonic_map;
    Solve(&harmonic_map);
  }
...
// -----------------------------------------------------------------------------
void irtkAsConformalAsPossibleVolumeParameterizer::Finalize()
{
  // Finalize base class
  irtkLinearVolumeParameterizer::Finalize();
...
// -----------------------------------------------------------------------------
static inline irtkMatrix3x3 GetScaleMatrix(const double n[3], double w)
{
  const double wn[3] = { w * n[0], w * n[1], w * n[2] };
	return irtkMatrix3x3(    .0,  wn[1], -wn[2],
                       -wn[0],     .0,  wn[2],
                        wn[0], -wn[1],     .0);
}

// -----------------------------------------------------------------------------
static inline irtkMatrix3x3 GetAngleMatrix(const double n[3], double w)
{
	const double wn[3] = { w * n[0], w * n[1], w * n[2] };
	return irtkMatrix3x3(   .0, wn[2], wn[1],
                       wn[2],    .0, wn[0],
                       wn[1], wn[0],    .0);
}

// -----------------------------------------------------------------------------
irtkMatrix3x3 irtkAsConformalAsPossibleVolumeParameterizer
                              ...
                              const double v2[3], const double v3[3], double volume) const
{
  ...
  vtkMath::Cross(a, b, n1);
	...
	irtkMatrix3x3 scale0 = GetScaleMatrix(n0, scale_weight);
	irtkMatrix3x3 scale1 = GetScaleMatrix(n1, scale_weight);
	irtkMatrix3x3 angle0 = GetAngleMatrix(n0, angle_weight);
	irtkMatrix3x3 angle1 = GetAngleMatrix(n1, angle_weight);

	irtkMatrix3x3 dTd = (scale0.Transpose() * scale1 + angle0.Transpose() * angle1);
  dTd /= 9.0 * volume;
