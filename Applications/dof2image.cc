...
void usage()
{
  ...
  cerr << "<-Ts value>                Time point of source image (4D/TD FFD)" << endl;
  exit(1);
}
...
int main(int argc, char **argv)
{
  ...
  double ts = .0; // Time point of source image
  ...
  while (argc > 1) {
    ...
    if ((ok == false) && (strcmp(argv[1], "-Ts") == 0)) {
      argc--;
      argv++;
      ts = atof(argv[1]);
      argc--;
  ...
  // Create images for displacements
  irtkImageAttributes attr = image.GetImageAttributes();

  irtkRealImage dx(attr);
  irtkRealImage dy(attr);
  irtkRealImage dz(attr);

  irtkRealImage d(attr, 3);
  for (t = 0; t < image.GetT(); t++) {
    double tt = image.ImageToTime(t);
    d = .0;
    if (invert) transform->InverseDisplacement(d, ts, tt);
    else        transform->Displacement       (d, ts, tt);
