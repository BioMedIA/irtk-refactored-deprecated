...
// -----------------------------------------------------------------------------
irtkImageAttributes
irtkFreeFormTransformation3D::DefaultAttributes(double x1, double y1, double z1,
                                                ...
                                                const double *zaxis)
{
  return irtkFreeFormTransformation::DefaultAttributes(
...
::irtkFreeFormTransformation3D(CPInterpolator &func, CPInterpolator *func2D)
:
  irtkFreeFormTransformation(func, func2D)
{
}

// -----------------------------------------------------------------------------
irtkFreeFormTransformation3D
::irtkFreeFormTransformation3D(const irtkFreeFormTransformation3D &ffd,
                               CPInterpolator &func, CPInterpolator *func2D)
:
  irtkFreeFormTransformation(ffd, func, func2D)
{
}

// -----------------------------------------------------------------------------
irtkFreeFormTransformation3D::~irtkFreeFormTransformation3D()
{
}

// -----------------------------------------------------------------------------
void irtkFreeFormTransformation3D::Initialize(const irtkImageAttributes &attr)
{
  if (attr._dt && attr._t != 1) {
    cerr << this->NameOfClass() << "::Initialize: Image attributes must represent a 3D image domain" << endl;
    exit(1);
  }
  irtkFreeFormTransformation::Initialize(attr);
}
...
// -----------------------------------------------------------------------------
/// (Multi-threaded) body of irtkFreeFormTransformation3D::ParametricGradient
///
/// This default implementation is more efficient than the general implementation
/// provided by irtkFreeFormTransformation for FFDs which are parameterized by
...
/// implementation given by irtkFreeFormTransformation::ParametricGradient or
/// provide their own specialized implementation.
class irtkFreeFormTransformation3DParametricGradientBody
{
public:
  const irtkFreeFormTransformation3D *_FFD;
  const irtkGenericImage<double>     *_Input;
  const irtkWorldCoordsImage         *_Image2World;
  const irtkWorldCoordsImage         *_WorldCoords;
  const irtkMatrix                   *_World2Image;
  ...
  void operator ()(const blocked_range3d<int> &re) const
  {
    ...
    irtkFreeFormTransformation3D::CPStatus status;

    // With transformed world coordinates
    if (_WorldCoords) {
      const irtkMatrix &w2i = *_World2Image; // approx. affine world to image space transformation
      ...
      for (int ci = re.cols ().begin(); ci != re.cols ().end(); ++ci) {
        ...
        i1 = static_cast<int>(round(x1 - _MarginX));
        i2 = static_cast<int>(round(x2 + _MarginX));
        j1 = static_cast<int>(round(y1 - _MarginY));
        j2 = static_cast<int>(round(y2 + _MarginY));
        k1 = static_cast<int>(round(z1 - _MarginZ));
        k2 = static_cast<int>(round(z2 + _MarginZ));
  ...
  void operator ()()
  {
    ...
    if (_Image2World && (_Image2World->X() != _X ||
                         ...
                         _Image2World->Z() != _Z ||
                         _Image2World->T() !=  3)) {
      cerr << "irtkFreeFormTransformation3D::ParametricGradient: Invalid voxel coordinates map" << endl;
      exit(1);
    }
    if (_WorldCoords && (_WorldCoords->X() != _X ||
                         ...
                         _WorldCoords->Z() != _Z ||
                         _WorldCoords->T() !=  3)) {
      cerr << "irtkFreeFormTransformation3D::ParametricGradient: Invalid world coordinates map" << endl;
      exit(1);
    }
    ...
    if (_WorldCoords) {
      ...
      irtkPointSet voxel(8), world(voxel.Size());
      int n = 0;
      for (int c = 0; c <= 1; ++c)
      for (int b = 0; b <= 1; ++b)
      for (int a = 0; a <= 1; ++a, ++n) {
        i = a * (_X - 1), j = b * (_Y - 1), k = c * (_Z - 1);
        voxel(n) = irtkPoint(i, j, k);
...
}; // irtkFreeFormTransformation3DParametricGradientBody

#ifdef HAS_VTK

// -----------------------------------------------------------------------------
class irtkFreeFormTransformation3DPointWiseParametricGradientBody
{
public:

  const irtkFreeFormTransformation3D *_Transformation;
  ...
  // ---------------------------------------------------------------------------
  /// Default constructor
  irtkFreeFormTransformation3DPointWiseParametricGradientBody()
  :
  ...
  // ---------------------------------------------------------------------------
  /// Copy constructor
  irtkFreeFormTransformation3DPointWiseParametricGradientBody(const irtkFreeFormTransformation3DPointWiseParametricGradientBody &other)
  :
  ...
  void operator ()(const blocked_range<int> &re) const
  {
    ...
    for (int cp = re.begin(); cp != re.end(); ++cp) {
      ...
      for (vtkIdType i = 0; i < ids->GetNumberOfTuples(); ++i) {
        ...
        const irtkVector3D<double> &g = _Input[idx];
        // Check whether reference point is valid
...
// -----------------------------------------------------------------------------
void irtkFreeFormTransformation3D
...
{
  IRTK_START_TIMING();
  irtkFreeFormTransformation3DParametricGradientBody body;
  body._FFD         = this;
  body._Input       = in;
  body._Output      = out;
  body._Weight      = w;
  ...
  body();
  IRTK_DEBUG_TIMING(2, "parametric gradient computation (3D FFD)");
}
                     ...
                     double *out, double t, double t0, double w) const
{
#ifdef HAS_VTK
  IRTK_START_TIMING();
  irtkFreeFormTransformation3DPointWiseParametricGradientBody body;
  ...
  body._t0             = t0;
  body();
  IRTK_DEBUG_TIMING(2, "point-wise parametric gradient computation (3D FFD)");
#else
...
// -----------------------------------------------------------------------------
irtkCifstream &irtkFreeFormTransformation3D::ReadDOFs(irtkCifstream &from, irtkTransformationType format)
{
  ...
  if (format == IRTKTRANSFORMATION_BSPLINE_FFD_3D_v1 || format == IRTKTRANSFORMATION_LINEAR_FFD_3D_v1) {
  ...
  if (static_cast<int>(format) >= 70) {
    ...
    if (mode >= Extrapolation_Last) {
      ...
      this->ExtrapolationMode(static_cast<irtkExtrapolationMode>(mode));
    }
  }
    ...
    for (int i = 0; i < attr._x; ++i) {
      for (int j = 0; j < attr._y; ++j) {
        for (int k = 0; k < attr._z; ++k) {
          _CPImage(i, j, k) = CPValue(param[dof    ],
                                      param[dof + 1],
                                      param[dof + 2]);
          dof += 3;
        }
...
// -----------------------------------------------------------------------------
irtkCofstream &irtkFreeFormTransformation3D::WriteDOFs(irtkCofstream &to) const
{
