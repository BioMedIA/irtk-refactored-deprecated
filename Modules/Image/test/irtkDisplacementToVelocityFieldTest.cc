...
// ---------------------------------------------------------------------------
void init(char *name_in, irtkImageAttributes &grid,
          double sigma, double max,
          double *&x,  double *&y,  double *&z,
          double *&dx, double *&dy, double *&dz)
{
  irtkGenericImage<irtkRealPixel> disp;

  if (name_in == NULL) {
    // create zero displacement field
    irtkImageAttributes attr = grid;
    attr._t = 3;
    disp.Initialize(attr);

    // add random noise to displacement field
    irtkGaussianNoise<irtkRealPixel> noise(0.0, sigma, -fabs(max), +fabs(max));
    noise.SetInput (&disp);
    noise.SetOutput(&disp);
    noise.irtkImageToImage<irtkRealPixel>::Run();

    // smooth displacements
    if (grid._z > 1) {
      irtkGaussianBlurring<irtkRealPixel> blur(1.25);
      blur.SetInput (&disp);
      blur.SetOutput(&disp);
      blur.Run();
    } else {
      irtkGaussianBlurring2D<irtkRealPixel> blur(1.25);
      blur.SetInput (&disp);
      blur.SetOutput(&disp);
      blur.Run();
    }
    ...
    irtkTransformation           *t    = irtkTransformation::New(name_in);
    irtkMultiLevelTransformation *mffd = dynamic_cast<irtkMultiLevelTransformation *>(t);
    irtkFreeFormTransformation   *ffd  = dynamic_cast<irtkFreeFormTransformation *>(t);
...
// ---------------------------------------------------------------------------
inline void jac(irtkMatrix &J, irtkGenericImage<double> &img, int i, int j, int k)
{
  J.Initialize(3, 3);
...
// ---------------------------------------------------------------------------
inline void expEuler(irtkGenericImage<double> &v, double &x, double &y, double &z, bool inv)
{
  ...
  irtkLinearInterpolateImageFunction vf;
  vf.SetInput(&v);
  vf.Initialize();
...
// ---------------------------------------------------------------------------
void testLogWithExpEuler(const irtkImageAttributes &grid,
                         ...
                         const double *dx, const double *dy, const double *dz)
{
  ...
  irtkGenericImage<double> phi(attr); // displacement field
  irtkGenericImage<double> v  (attr); // current velocity field
  irtkGenericImage<double> vn (attr); // next velocity field
  irtkGenericImage<double> dv (attr); // dv = exp(-vp) ° Phi(x) - x
  ...
  for (int s = 0; s < nsteps; s++) {
    ...
    irtkGaussianBlurring2D<double> blur(2.0 * (grid._dx > grid._dy ? grid._dx : grid._dy));
    blur.SetInput (&dv);
    blur.SetOutput(&dv);
    blur.Run();
  ...
  irtkBSplineFreeFormTransformation3D ffd(const_cast<irtkImageAttributes&>(grid), grid._dx, grid._dy, grid._dz);
  ...
  ffd.irtkTransformation::Write("test1-vffd1.dof.gz");
  ...
  ffd.irtkTransformation::Write("test1-vffd2.dof.gz");
...
int irtkDisplacementToVelocityFieldTest(int ac, char *av[])
{
  ...
  irtkImageAttributes attr = grid; attr._t = 3;
  irtkGenericImage<double> din(attr);
  ...
  irtkVelocityToDisplacementFieldEuler<double> vtod;
  irtkGenericImage<double>                     dout;
  ...
  if (dofout != NULL) {
    if (strstr(dofout, ".dof") != NULL) {
      ...
      irtkFreeFormTransformation3D *ffd = new irtkLinearFreeFormTransformation(dout, grid._dx, grid._dy, grid._dz);
